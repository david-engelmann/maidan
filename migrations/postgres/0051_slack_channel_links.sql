-- Slack projector channel links (Cluster 308): maps a Slack channel to the Maidan
-- channel/thread it projects into, and the member inbound Slack messages are posted
-- as. One Maidan thread per Slack channel (the projected conversation).
CREATE TABLE IF NOT EXISTS maidan_slack_channel_links (
    slack_channel_id TEXT PRIMARY KEY,
    workspace_id UUID NOT NULL REFERENCES maidan_workspaces(id) ON DELETE CASCADE,
    channel_id UUID NOT NULL REFERENCES maidan_channels(id) ON DELETE CASCADE,
    thread_id UUID NOT NULL REFERENCES maidan_threads(id) ON DELETE CASCADE,
    member_id UUID NOT NULL REFERENCES maidan_members(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_slack_links_workspace
    ON maidan_slack_channel_links (workspace_id);

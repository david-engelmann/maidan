...
- **Release cadence:** work ships in clusters — the initial A–H + 1.0
  ...
  **`maidan-scale-1.0`** at **`v120.0.0`**. No further *ladder* cluster
  ...
  (Phase XXIV, **Cluster 121+**, latest **`v130.0.0`**, tagged `vX.0.0` on
...
- **Do not bypass GPG signing** unless explicitly authorized. No
  signing key is configured (tags through `v130.0.0` are annotated but
...
- **No `v93`–`v100` tags (intentional):** clusters **93–101** shipped as a single batch PR (#264) and were released as **`v101.0.0`** — they were never separate releases, so there are no `v93.0.0`–`v100.0.0` tags to cut. Version tags cut: `v101.0.0`, `v102.0.0`–`v120.0.0`, and `v121.0.0`–`v130.0.0`.

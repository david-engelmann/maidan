...
| Thread run budget | `PUT`/`PATCH`/`GET /threads/:id/budget`, MCP `set_thread_budget`/`update_thread_budget`/`get_thread_budget` | Token, USD-micros, turn, and working-wall limits. PUT/set is a total replacement (all four dimensions required; `null` = uncapped); PATCH/update is an atomic partial merge (absent = unchanged, `null` = clear). Accumulated usage survives either operation |

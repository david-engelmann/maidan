...
| Land-gate pointer | `/threads/:id/land-gate` + `/land-gate/requirement`, MCP `set/get/require/clear_land_gate` | Opt-in close-gate (no row = vacuous green). A qualifying land is a **green pass** from a `land_gate`-skilled member who is neither the owner, the current assignee, nor anyone who *ever held* the thread (Cluster 401.2 — testing only the live assignee let an implementer release the claim and then pass their own work). Granting the `land_gate` skill needs `channel:admin` (Cluster 400.5), so the skill is a real qualification rather than something an agent can give itself. Amber (flags-then-still-engages) is not a land; fail is always red. Room holds the pointer; an external verifier records pass/fail. Cluster 385. |

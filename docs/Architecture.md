...
| Land-gate pointer | `/threads/:id/land-gate` + `/land-gate/requirement`, MCP `set/get/require/clear_land_gate` | Opt-in close-gate (no row = vacuous green). A qualifying land is a **green pass** from a `land_gate`-skilled member ≠ owner/assignee — where *skilled* means **declared**, and declaring is currently `workspace:write` and self-service, so separation of duties is the binding half (see [Open Work](Open%20Work.md)). Amber (flags-then-still-engages) is not a land; fail is always red. Room holds the pointer; an external verifier records pass/fail. Cluster 385. |

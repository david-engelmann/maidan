...
| Thread run budget | `PUT`/`PATCH`/`GET /threads/:id/budget`, `POST /threads/:id/usage`; MCP `set_thread_budget`/`update_thread_budget`/`get_thread_budget`/`report_usage` | Token, USD-micros, turn, and working-wall limits. PUT/set is total; PATCH/update is a partial merge. Accounted usage derives reporter/payer, is claim-lease fenced and report-id idempotent, and atomically binds its immutable price snapshot, ledger row, accumulated totals, `UsageReported`, and any over-budget claim failure |
...
| Events | `GET /workspaces/:wid/events`, `GET …/events/verify`, `GET …/snapshot`, `GET …/events/catch-up`, outbox admin routes | Replay + hash-chain integrity + hashed snapshot / since-LSN catch-up + quarantined-outbox list/replay; `contracts/event-surface-disposition.json` exhaustively records which kinds REST, MCP, both, or internal workers can produce |
...
- **Authorization evidence.** REST and MCP capability decisions emit one
  content-free record with principal, action, outcome, and resource identity.

...
| Required reviewers | `/threads/:id/review-requirement` + `/reviewers` + `/reviews` + `/review-status`, MCP `set_review_requirement`/`add_reviewer`/`submit_review`/`get_review_status` | A thread's `closed` transition is gated on `k` distinct qualifying approvals (reviewer ≠ owner/assignee — separation of duties) + no unresolved `refutes` edge; a gate, not a poll/closer |

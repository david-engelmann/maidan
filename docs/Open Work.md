...
- **Latest tag:** **`v190.0.0`** (post-gate hardening, Phase XXIV). Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189). **Arc C — agentic task-queue depth** began: 190 assignment read-side (list-mine + atomic claim-next, REST). Arc C remaining: MCP tools for the read-side, claim leases + reclaim, `roots/list` tool, structured tool-call transcripts, `wait_for_mention`, handoff notes, federation `parts→content`. Then Arc D (performance & scale).
- **Assignment read-side follow-ups (Cluster 190):** MCP tools for `list_assigned_threads` / `claim_next_thread` (the member-scoped list needs its own MCP-side RBAC filter — the pre-dispatch channel-access gate keys on channel/thread args, not `member_id`); no claim **lease** (a claimed-then-dead agent holds the thread forever — lease + reclaim is the next Arc-C cluster); `claim_next` is channel-scoped (no workspace-wide pull).

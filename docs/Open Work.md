...
- **Latest merged: Cluster 368 (Wave 2 #16 — the waiting-on-you inbox) on `main`; tags `v350.0.0`–`v368.0.0` pending the maintainer.** A stacked cluster (PRs #726 aggregate+REST, #727 MCP, #729 `/ui`) — see [[Retros/Cluster 368]]. **Wave 1 (#1–14) + Wave 2 #15–16 are COMPLETE. Next: Wave 2 #17** (an AG-UI door on the WS/SSE, H1). *(v350–v368 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)* The 2026-09 **Pi-integration-prep review** confirmed Wave 1/2 are honestly closed (no parallel "Pi prep" backlog); its one fold — a doc-only `waiter-loop-recipe` + lazy-`ClaimExpired` note — is tracked under Program U.

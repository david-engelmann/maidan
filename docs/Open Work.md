...
- **Latest merged: Cluster 372 (Wave 2 #20 — a freeze-member kill-switch, G17+B25) on `main`; tags `v350.0.0`–`v372.0.0` pending the maintainer.** Four impl PRs (#747 store, #748 claim enforcement, #749 REST, #750 MCP) + a retro — see [[Retros/Cluster 372]]. Freezing a member drops their leases + refuses `claim_next` (atomic); REST + MCP (`token:admin`, audited) + a "Kill switches" operator catalog in Operations.md. The freeze is the gate; not G4 PAUSE. **Wave 1 (#1–14) + Wave 2 #15–20 are COMPLETE. Next: Wave 2 #21** (H11 — attachable labeled memory as room objects, the Letta-block shape `{label, description, limit, read_only, value}`; attach/detach; a parent watches a child's result block without a nested runtime). *(v350–v372 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)* The 2026-09 **Pi-integration-prep review** confirmed Wave 1/2 are honestly closed (no parallel "Pi prep" backlog); its one fold — a doc-only `waiter-loop-recipe` + lazy-`ClaimExpired` note — is tracked under Program U. **The `context_query_count_e2e` connection-warm-up flake was fixed (PR #746, a pool warm-up before the measured builds).**

...
- **Latest tag:** **`v196.0.0`** (post-gate hardening, Phase XXIV). Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189). **Arc C — agentic task-queue depth** underway: 190 assignment read-side (REST), 191 (MCP tools), 192 claim leases + reclaim, 193 `list_roots` tool, 194 A2A `parts→content` ingest, 195 handoff notes on assignment, 196 `wait_for_mention` blocking MCP long-poll. Arc C remaining: structured tool-call transcripts; federation `content→parts` **egress** (194 did ingest; egress still body-only). Then Arc D (performance & scale).

...
5. **H4** — lookback on `wait_for_*` + evict-on-wait; side effects before a wait must be idempotent (resume replays); no occupancy I/O in Drop. **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 353 (the identity chrome) on `main`; tags `v350.0.0`–`v353.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, Cluster 350, PRs #612–#619), #2 (occupancy clocks, Cluster 351, PRs #620–#626), #3 (HITL list, Cluster 352, PRs #628–#631), and #4 (identity chrome, Cluster 353, PRs #633–#636) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]]). The vanilla `/ui` now shows a session's real capabilities, per-thread session chrome, and attenuation, keyboard-operable to WCAG AA. **Next up: Wave 1 #5 — H4** (lookback on `wait_for_*` + evict-on-wait; idempotent side effects before a wait; no occupancy I/O in Drop). *(v350–v353 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

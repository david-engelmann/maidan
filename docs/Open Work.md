...
12. **G-dev-7** — inbound `pull_request.merged` → `ThreadLanded` (steal the landed fact, not an automation product; projector is `issue_comment` + `ping` only today). **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 360 (the token-budgeted context pack) on `main`; tags `v350.0.0`–`v360.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, 350), #2 (occupancy clocks, 351), #3 (HITL list, 352), #4 (identity chrome, 353), #5 (wait contract, 354), #6 (owner/steer, 355), #7 (threading, 356), #8 (scoped notification mute, 357), #9 (the budget envelope, 358), #10 (inbox & search depth, 359), and #11 (the token-budgeted context pack, Cluster 360, PRs #668/#671/#673/#672) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]], [[Retros/Cluster 354]], [[Retros/Cluster 355]], [[Retros/Cluster 356]], [[Retros/Cluster 357]], [[Retros/Cluster 358]], [[Retros/Cluster 359]], [[Retros/Cluster 360]]). The context pack now caps itself by tokens (keep the framing + recent tail, fold the middle into an auditable `elision`) and a child thread's pack grounds on its parent's ask + decision. **Next up: Wave 1 #12 — G-dev-7** (inbound `pull_request.merged` → `ThreadLanded`). *(v350–v360 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

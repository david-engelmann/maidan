...
- **Latest merged: Cluster 370 (Wave 2 #18 — a recipe / thread-type, G8+W5+G-dev-9) on `main`; tags `v350.0.0`–`v370.0.0` pending the maintainer.** Five impl PRs (#735 store, #736 instantiate, #737 REST, #738 MCP, #739 scheduler) + a retro — see [[Retros/Cluster 370]]. A recipe is a reusable thread-type blueprint; `instantiate_recipe` builds parent + DAG children + skills + a copy-on-fire snapshot; a `task_schedules.recipe_id` seeds a run per firing (`ScheduleSkipped` when the prior run is in flight). Not a recipe VM. **Wave 1 (#1–14) + Wave 2 #15–18 are COMPLETE. Next: Wave 2 #19** (G19 + T3 — secret-ref: the log holds an id, the store holds the value, Pi fetches at exec; a SecretBroker substitutes on egress with a host allowlist). *(v350–v370 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)* The 2026-09 **Pi-integration-prep review** confirmed Wave 1/2 are honestly closed (no parallel "Pi prep" backlog); its one fold — a doc-only `waiter-loop-recipe` + lazy-`ClaimExpired` note — is tracked under Program U.

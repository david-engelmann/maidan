...
- **Latest merged: Cluster 364 (wait-edges + escalation) on `main`; tags `v350.0.0`–`v364.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, 350), #2 (occupancy clocks, 351), #3 (HITL list, 352), #4 (identity chrome, 353), #5 (wait contract, 354), #6 (owner/steer, 355), #7 (threading, 356), #8 (scoped notification mute, 357), #9 (the budget envelope, 358), #10 (inbox & search depth, 359), #11 (the token-budgeted context pack, 360), #12 (the landed fact, 361), and #13 in full — G11 hard WIP (362), G3 Unclaimable (363), and G2/G4 wait-edges + escalation (Cluster 364, PRs #698/#701/#704/#705/#706) — are all SHIPPED** ([[Retros/Cluster 350]]–[[Retros/Cluster 364]]). A thread can now declare a durable wait timer with an `on_timeout` policy (`Notify`/`Park`) fired by an opt-in sweeper → `WaitTimedOut` → the owner is reached; a thread can be parked from dispatch; the WIP limit caps concurrent live claims — all over REST + MCP. **Next up: G3 fair dispatch** (anti-starvation `claim_next` ordering; the last sub-item of #13) — then **#14** (N1/T6/H15/SCIM). *(v350–v364 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

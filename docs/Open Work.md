...
- **Latest merged: Cluster 371 (Wave 2 #19 — secret-ref, G19+T3) on `main`; tags `v350.0.0`–`v371.0.0` pending the maintainer.** Four impl PRs (#741 store, #742 REST, #743 MCP, #744 egress broker) + a retro — see [[Retros/Cluster 371]]. A named secret whose value never enters the event log — the log holds a `secret://<name>` reference, `maidan_secrets` holds the encrypted value; REST/MCP `resolve` (Pi fetches at exec) + an egress `SecretBroker` (allowlisted hosts only). **Wave 1 (#1–14) + Wave 2 #15–19 are COMPLETE. Next: Wave 2 #20** (G17 + B25 — a freeze-member kill-switch: drop leases, refuse `claim_next`, leave a gate; + a catalog of `MAIDAN_*` kill-switch env/flags). *(v350–v371 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)* The 2026-09 **Pi-integration-prep review** confirmed Wave 1/2 are honestly closed (no parallel "Pi prep" backlog); its one fold — a doc-only `waiter-loop-recipe` + lazy-`ClaimExpired` note — is tracked under Program U. **Known flake:** `context_query_count_e2e` (±1 on a connection warm-up) recurs on the integration/coverage jobs — a pool-warm-up fix is a tracked chore.

...
- **Current baseline (superseding the retained chronology below): Cluster 408 (full-audit remediation), released as `v408.0.0`.** Five implementation PRs (#964/#973/#974/#975/#976) self-scope personal member state, bound client inputs, centralize safe outbound HTTP, harden deployment defaults, make `/ui` states honest, repair published rendering and presentation, and add a flag-off advisory land-gate measurement path with no write authority. See [[Clusters/Cluster 408]] and [[Retros/Cluster 408]]. **Prior: Cluster 407 is released as `v407.0.0`.**

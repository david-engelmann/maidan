...
`run_id` is the first real producer — accepted as-is, not reminted; follow-occupancy + manager digest remain); row #25 shipped as Cluster 383 (composition) + Cluster 385 (pointer + land vocabulary). **Plus one that has no existing row:** pi's

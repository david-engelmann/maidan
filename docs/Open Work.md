...
39. **B9 + B13 + B4 + B23 — IN PROGRESS (Cluster 407, #963).** **407.1:** an executable `/ui` route census derives templates from the embedded JavaScript and checks them against OpenAPI plus the mounted session-proxy methods (`NEW-ui-fetch-contract`). Remaining: an EventKind × REST/MCP matrix CI (`NEW-migration-parity-gate` sibling); deeper in-process `ui_*` hero e2e; golden export-frame fixtures. [[Clusters/Cluster 407]].

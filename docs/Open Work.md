...
- **Legal-hold follow-ups** (after the preservation decision): (1) **holds per matter** — one row per workspace today, so a second matter overwrites the first and one lift ends both (M); (2) **withdrawn bodies in the event log** — ~~member-facing~~ **✅** the events list reads a withdrawn message's posted and edited events back with the body blank; the whole-log tier (admins' catch-up, peers) still carries the words, so GDPR purge needs crypto-shredding there, a decision (L); (3) delete a withdrawn message's embeddings (S)

...
- **P1.1d — no MCP `transition_thread` twin. ⏳ P1 (confirm intent).** REST has `transition_thread`
  ...
  **Don't be fooled by the capability name (verified 2026-09-12):** 23 MCP tools require
  `thread:transition`, including #22's `set_review_requirement`/`add_reviewer`/`submit_review` — but
  none of them transitions the FSM. The capability is the grant label for "may change a thread's
  disposition" (claim lifecycle, owner, result, budget, priority, review decisions); no MCP tool calls
  `transition_thread_with_event`. So #22 shipping review tools under that capability did **not** close
  P1.1d, and an MCP-only agent still cannot close its own thread.
...
- ~~**Cluster 385 — Soundcheck gate pointer + green/amber/red land vocabulary** *(row #25 remainder)*~~ **✅ SHIPPED (`v385.0.0`), PRs #819/#821/#826/#827 + retro** ([[Retros/Cluster 385]]). Opt-in close-gate (no row = vacuous green). Qualifying land = green pass from a `soundcheck`-skilled member ≠ owner/assignee. Amber is not a land. Fail is always red. Room holds the pointer; Soundcheck owns test execution. Cluster 384 is P1.1d. **Row #25 is closed.**

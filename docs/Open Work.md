...
- **Latest tag:** **`v192.0.0`** (post-gate hardening, Phase XXIV). Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189). **Arc C — agentic task-queue depth** underway: 190 assignment read-side (REST), 191 (MCP tools), 192 claim leases + reclaim. Arc C remaining: `roots/list` tool, structured tool-call transcripts, `wait_for_mention`, handoff notes, federation `parts→content`. Then Arc D (performance & scale).

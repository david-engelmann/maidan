...
- **Legal-hold follow-ups** (after the preservation decision): (1) **holds per matter** — one row per workspace today, so a second matter overwrites the first and one lift ends both (M); (2) **withdrawn bodies in the event log** — `MessagePosted`/`MessageEdited` payloads keep a withdrawn message's words; the member-facing events list should redact them (S, builds on the event-visibility filter), and the whole-log tier (admins, peers) needs crypto-shredding for GDPR purge to be real, a decision (L); (3) delete a withdrawn message's embeddings (S)

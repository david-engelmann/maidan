...
- **Current baseline (superseding the retained chronology below): Cluster 405 (time-boxed cross-organization incident sharing); target tag `v405.0.0`.** Three implementation PRs (#952/#953/#954) close Wave 2 row #26 with a dual-backend ticket ledger, audited REST/MCP issuer lifecycle, and a distinct read-only `ShareTicket` consumer boundary. See [[Clusters/Cluster 405]] and [[Retros/Cluster 405]]. **Prior: Cluster 404 is released as `v404.0.0`.**

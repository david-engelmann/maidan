...
6. **W1 = G-dev-2 + G-dev-4** — a durable human **owner** ≠ claimer; the claimer cannot merge its own PR; persist steer; notify the owner on stuck. An auto-reviewer subagent is not the owner (`owner_id` empty today). **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 354 (the wait contract) on `main`; tags `v350.0.0`–`v354.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, Cluster 350, PRs #612–#619), #2 (occupancy clocks, Cluster 351, PRs #620–#626), #3 (HITL list, Cluster 352, PRs #628–#631), #4 (identity chrome, Cluster 353, PRs #633–#636), and #5 (wait contract, Cluster 354, PRs #638–#640) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]], [[Retros/Cluster 354]]). The `wait_for_*` long-polls now take an opt-in `since_log_id` lookback that closes the drain/subscribe race. **Next up: Wave 1 #6 — W1** (a durable human owner ≠ claimer; the claimer cannot merge its own PR; persist steer; notify the owner on stuck). *(v350–v354 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

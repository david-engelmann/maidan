...
- **NEW four-program arc (from a 5-agent sweep, 2026-08-12) — run in order, clusters 202+:** **(A) Security & correctness round 2** — 202 session-bound acting identity (anti-spoofing) ✅; remaining: **203** DM/group-DM participation on subscribe + metadata reads (the 180 gap's events/metadata analog — `dm.rs:63 expand_event_filter` has no participant check), **204** cross-tenant artifact isolation (`Artifact` has no `workspace_id`; a known SHA + `workspace:read` reads another tenant's blob — add a `maidan_artifact_refs` link table + ref-counted purge), **205** transactional outbox (atomic domain-write + event-append — the 184 deferral; `*_with_event` Store methods in one tx), **206** federation ingest trust policy (event-kind allowlist + referenced-entity-in-peer-workspace) + an optional RLS spike. **(B) agentic orchestration** (task DAG, scheduled/recurring tasks, capability registry + skill routing, queue depth, coordination waits + structured results). **(C) notifications & reach** (per-recipient router + inbox, prefs + presence-aware routing, email/SMTP transport, digests, follow/UI). **(D) scale & durability** (read-replica routing, transactional outbox [shared w/ A], backup/DR, chaos/fault-injection, event-bus self-healing NOTIFY floor). Full per-lens detail in the session's workflow journal `wf_b8cdaaa2-be4`.

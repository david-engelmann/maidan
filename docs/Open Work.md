...
- **Latest merged: Cluster 367 (Wave 2 #15 — the human work console) on `main`; tags `v350.0.0`–`v367.0.0` pending the maintainer.** A stacked `/ui` cluster (PRs #722 Work tab, #723 Prefs console, #724 looking glass) — see [[Retros/Cluster 367]]. **Wave 1 (#1–14) + Wave 2 #15 are COMPLETE. Next: Wave 2 #16** (a waiting-on-you inbox — G15 + G9). *(v350–v367 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

...
- ~~**Cluster 383 — critical → `request_changes` adapter** *(row #25 composition; not a new gate)*~~ **✅ SHIPPED (`v383.0.0`), PRs #809/#811/#812/#813** ([[Retros/Cluster 383]]). A reviewed `pi.review.result/1` with any `critical` finding from a review-skilled producer writes Cluster-375 `request_changes` and arms `k=1` when unset. Empty `deliver_to` still arms. Never auto-approve. GitHub `event` stays `COMMENT`. Soundcheck pointer + green/amber/red remain on #25.

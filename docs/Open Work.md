...
34. ~~**B8 + B27** — a tombstone/deletion explorer + a backlink index ("what points at this message") + a kind census.~~ **✅ SHIPPED (`v394.0.0`), PRs #859/#860/#861 + this retro** ([[Retros/Cluster 394]]). Tombstones (`GET /workspaces/:id/tombstones` + MCP `list_tombstones`; `include_purged` reconstructs hard deletes from `MessageTombstoned`). Backlinks (`GET /messages/:id/backlinks` + MCP `list_message_backlinks` — Cluster 320 reverse edges + pins/reactions/votes; mentions outgoing, omitted). Kind census (`GET /workspaces/:id/kind-census` + MCP `get_kind_census`; `private_channel_deny_set` in the query). No new table. **Row #34 is closed.** Do not start #35–36 from this close.

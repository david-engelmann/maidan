...
- **Latest merged: Cluster 369 (Wave 2 #17 — an AG-UI door, H1) on `main`; tags `v350.0.0`–`v369.0.0` pending the maintainer.** Two impl PRs (#731 types+mapping, #733 SSE endpoint) + a retro — see [[Retros/Cluster 369]]. A thread is a run; `GET /agui/stream` emits AG-UI frames off the resumable bus (`Last-Event-ID` resume, per-event RBAC, off-contract like `/mcp/stream`); output direction only. **Wave 1 (#1–14) + Wave 2 #15–17 are COMPLETE. Next: Wave 2 #18** (a recipe / thread-type — G8 + W5 + G-dev-9). *(v350–v369 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)* The 2026-09 **Pi-integration-prep review** confirmed Wave 1/2 are honestly closed (no parallel "Pi prep" backlog); its one fold — a doc-only `waiter-loop-recipe` + lazy-`ClaimExpired` note — is tracked under Program U.

...
3. **H12** — the HITL list, EXPAND 282–289 (see EXPAND). **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 351 (occupancy clocks) on `main`; tags `v350.0.0` + `v351.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, Cluster 350, PRs #612–#619) and #2 (occupancy clocks, Cluster 351, PRs #620–#626) are both SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]]) — the two "standing hole" rows are closed. **Next up: Wave 1 #3 — H12, the HITL `tasks/list` remainder** (so an external A2A agent can discover a pending held gate). *(v350/v351 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

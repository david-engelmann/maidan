...
- **HITL list — 282–289 follow-up ← H12 (Wave 1 #3). ✅ SHIPPED — Cluster 352 (`v352.0.0`), PRs #628–#631**
  ...
  a2a-tck MUST failures as a Soundcheck `/test` (`NEW-a2a-tck-ci`).
...
24. ~~**G7 + G-dev-10** — the `claim_next` pack includes in-channel **accepted decisions**, and `result_kind` is a search facet~~ **✅ SHIPPED — Cluster 382 pack (`v382.0.0`, PRs #792/#797/#796) + Cluster 381 facet (`v381.0.0`, PRs #798/#799/#801/#802)** ([[Retros/Cluster 382]], [[Retros/Cluster 381]]). **Pack (382):** live `get_thread_context` (REST + MCP) carries token-lean `accepted_decisions` teasers for closed/archived in-channel results so the next `claim_next` claimer sees what the channel already decided. Waiter envelopes (`schema = pi.waiter.result/1`) appear only when `status` is `reviewed`; opaque JSON on a terminal thread is treated as accepted. `claim_next` itself still returns `Option<Thread>` — the pack is the claimer's read. Default on; `include_accepted_decisions=false` drops it; withheld on DM / as-of / workspace-nested packs. **Facet (381):** `result_kind` is a **namespaced string** (e.g. `pi.review.result/1`), not a closed enum — the 2026-09-12 correction, now shipped. Indexed on `maidan_thread_results` (pg 0087 / sqlite 0086); listed over `GET /workspaces/:id/results?result_kind=` + MCP `list_thread_results` (`workspace:read`; exact match; omit = every accessible non-tombstoned result; private-channel rows dropped). This is a thread-result **list**, not `GET …/search`. The ADR convention `"kind": "decision"` is not a filter value. **Row #24 is closed.**
25. ~~**G-dev-6** — a **Soundcheck gate pointer** on the thread (`{kind:"soundcheck", status:pass|fail, artifact_sha?}`): the FSM will not `closed` without a pass from a soundcheck-skilled member ≠ the implementer, and a **green/amber/red** land-gate vocabulary so the FSM will not `closed` on **accepted nonsense** (amber = flags-then-still-engages is not a land). The room holds the pointer; Soundcheck owns `/test`; Pi may run the MIT BullshitBench dataset. Not a CI product / a judge panel in the room.~~ **✅ SHIPPED — Cluster 383 composition (`v383.0.0`, PRs #809/#811/#814/#815) + Cluster 385 pointer (`v385.0.0`, PRs #819/#821/#826/#827/#831)** ([[Retros/Cluster 383]], [[Retros/Cluster 385]]). Composition: a reviewed `pi.review.result/1` with any `critical` finding from a review-skilled producer writes Cluster-375 `request_changes` and arms `k=1` when unset. Pointer: a thread-scoped `{kind:"soundcheck", status, artifact_sha?, land}` row; `closed` refuses unless a **green pass** from a `soundcheck`-skilled member ≠ owner/assignee. Amber (flags-then-still-engages) is not a land. Fail is always red. No row is vacuous green (additive, Cluster 375 shape). Room holds the pointer; Soundcheck owns test execution. Not a CI product / a judge panel. **Row #25 is closed.**
...
- ~~**Cluster 381 — `result_kind` facet + the pinned spec** *(facet half of row #24; the pack half shipped as Cluster 382)*~~ **✅ SHIPPED (`v381.0.0`), PRs #798/#799/#801/#802** ([[Retros/Cluster 381]]). **381.1** `result_kind_from_payload` + indexed column on `maidan_thread_results` (pg 0087 / sqlite 0086) + `list_thread_results` both backends (382's `list_channel_closed_results` untouched). **381.2** `GET /workspaces/:id/results?result_kind=` (`workspace:read`; `can_access_thread` post-filter; full new-route preflight). **381.3** MCP `list_thread_results` (workspace from the token). **381.4** Integration + Result Delivery — discoverability is the namespaced string; `"kind": "decision"` is not a filter value (**not a retro**). Facet on the **namespaced string** (exact match; never a closed enum; not message-FTS). **Row #24 is closed.** Registering `pi.waiter.result/1` in the Wave 3 #30 schema pack stays Wave 3 — this cluster did not invent that pack.
- ~~**Cluster 383 — critical → `request_changes` adapter** *(row #25 composition; not a new gate)*~~ **✅ SHIPPED (`v383.0.0`), PRs #809/#811/#814/#815** ([[Retros/Cluster 383]]). A reviewed `pi.review.result/1` with any `critical` finding from a review-skilled producer writes Cluster-375 `request_changes` and arms `k=1` when unset. Empty `deliver_to` still arms. Never auto-approve. GitHub `event` stays `COMMENT`. Soundcheck pointer + green/amber/red shipped as Cluster 385.
- ~~**Cluster 385 — Soundcheck gate pointer + green/amber/red land vocabulary** *(row #25 remainder)*~~ **✅ SHIPPED (`v385.0.0`), PRs #819/#821/#826/#827/#831** ([[Retros/Cluster 385]]). Opt-in close-gate (no row = vacuous green). Qualifying land = green pass from a `soundcheck`-skilled member ≠ owner/assignee. Amber is not a land. Fail is always red. Room holds the pointer; Soundcheck owns test execution. Cluster 384 is P1.1d. **Row #25 is closed.**

...
- **Latest merged: Cluster 352 (the HITL list) on `main`; tags `v350.0.0` + `v351.0.0` + `v352.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, Cluster 350, PRs #612–#619), #2 (occupancy clocks, Cluster 351, PRs #620–#626), and #3 (HITL list, Cluster 352, PRs #628–#631) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]]). An external A2A agent can now discover a pending held gate via `tasks/list?status=input-required`. **Next up: Wave 1 #4 — N8 / B1 + N7** (session chrome running/idle/needs-input/needs-approval/done + a capability card, and flagship buttons in the vanilla `/ui` with WCAG AA; no SPA). *(v350/v351/v352 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

...
- **Latest tag:** **`v191.0.0`** (post-gate hardening, Phase XXIV). Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189). **Arc C — agentic task-queue depth** underway: 190 assignment read-side (REST), 191 assignment read-side (MCP tools). Arc C remaining: claim leases + reclaim, `roots/list` tool, structured tool-call transcripts, `wait_for_mention`, handoff notes, federation `parts→content`. Then Arc D (performance & scale).
- **Assignment read-side follow-ups (Clusters 190–191):** MCP tools shipped in 191 (member-scoped `list_assigned_threads` is RBAC-filtered in-handler like `search_messages`). Remaining: no claim **lease** (a claimed-then-dead agent holds the thread forever — lease + reclaim is the next Arc-C cluster); `claim_next` is channel-scoped (no workspace-wide pull).

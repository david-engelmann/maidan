...
7. **F1 + F2 + F7** — every post is a titled thread; collapsed children; leaf mute; `ThreadBumped`. Also Automerge as *thread* collab (causal edits on the same titled thread — **the event log stays the log**, do not CRDT the log). **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 355 (the owner/steer cluster) on `main`; tags `v350.0.0`–`v355.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, 350), #2 (occupancy clocks, 351), #3 (HITL list, 352), #4 (identity chrome, 353), #5 (wait contract, 354), and #6 (owner/steer, Cluster 355, PRs #642–#646) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]], [[Retros/Cluster 354]], [[Retros/Cluster 355]]). A task thread now has a durable owner (≠ claimer), the claimer can't land its own owned work, steering persists across handoffs, and the owner is notified on stuck — over REST + MCP. **Next up: Wave 1 #7 — F1 + F2 + F7** (every post a titled thread; collapsed children; leaf mute; `ThreadBumped`). *(v350–v355 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

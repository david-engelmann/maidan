...
13. **G2 / G4 / G3 / G11** — wait-edges + `on_timeout`; an escalation enum (G4 **after** the held gate in #1); fair dispatch + Unclaimable + hard WIP. Steal the Restate/Temporal promise/timer *shape* onto wait-edges; do not become the engine. **← NEXT (the current next-ranked feature).**
...
- **Latest merged: Cluster 361 (the landed fact) on `main`; tags `v350.0.0`–`v361.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, 350), #2 (occupancy clocks, 351), #3 (HITL list, 352), #4 (identity chrome, 353), #5 (wait contract, 354), #6 (owner/steer, 355), #7 (threading, 356), #8 (scoped notification mute, 357), #9 (the budget envelope, 358), #10 (inbox & search depth, 359), #11 (the token-budgeted context pack, 360), and #12 (the landed fact, Cluster 361, PRs #675/#678/#680/#681) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]], [[Retros/Cluster 354]], [[Retros/Cluster 355]], [[Retros/Cluster 356]], [[Retros/Cluster 357]], [[Retros/Cluster 358]], [[Retros/Cluster 359]], [[Retros/Cluster 360]], [[Retros/Cluster 361]]). The GitHub projector now steals the landed fact — a merged PR linked to a thread emits a `ThreadLanded` event that reaches the owner + followers and can be awaited over MCP (`wait_for_landed`), without touching the FSM. **Next up: Wave 1 #13 — G2 / G4 / G3 / G11** (wait-edges + `on_timeout` + escalation + fair dispatch). *(v350–v361 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

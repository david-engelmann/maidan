...
39. **B9 + B13 + B4 + B23 — IN PROGRESS (Cluster 407, #963).** **407.1:** an executable `/ui` route census derives templates from the embedded JavaScript and checks them against OpenAPI plus the mounted session-proxy methods (`NEW-ui-fetch-contract`). **407.2:** every `EventKind` has a machine-readable REST-only/MCP-only/both/internal-only disposition with executable evidence; the audit also repaired MCP artifact uploads that omitted `ArtifactUpserted` (`NEW-migration-parity-gate` sibling). **407.3:** one causal e2e uses a signed browser session for `/ui/api` channel/thread/message writes and proves the exact message arrives on the already-acknowledged live WebSocket. **407.4:** normalized goldens lock the signed-export envelope and snapshot/catch-up frame shapes without freezing generated UUIDs, timestamps, hashes, keys, or signatures. Implementation is complete; close record and `v407.0.0` remain. [[Clusters/Cluster 407]].
...
- **Current baseline (superseding the retained chronology below): Cluster 406 (published boot proof and real loopback OIDC), released as `v406.0.0`.** Three implementation PRs (#957/#961/#959) close Wave 4 row #37 with an exact-bundle CLI image, a pull-by-tag release gate, and the production OIDC path exercised against a real cryptographic loopback provider. See [[Clusters/Cluster 406]] and [[Retros/Cluster 406]]. **Prior: Cluster 405 is released as `v405.0.0`.**

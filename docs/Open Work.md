...
36. **B10** — a **WASI slash-handler kind** (a workspace installs a no-network guest with gas/memory caps; Maidan stores + invokes; the guest is the tool). Giant, still a room. Not an agent runtime (that's an external agent runtime).

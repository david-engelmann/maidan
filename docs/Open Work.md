...
| ~~413.3b~~ | ~~**D-A:** workspace purge, erase and import, message purge, legal hold; reads~~ **✅ 413.3b** — all commit with their row; export and secret resolve are withheld if their row cannot be written. Also fixed: MCP forced restore bypassed the legal hold; a forced restore was not atomic; erasing a workspace anonymized its audit history. ~~**Decision for the maintainer:** tombstones under a hold~~ **✅ decided 2026-09-25 (David: "the best decision possible based on research")** — under a hold a withdrawn message keeps its words and earlier versions, copied to `maidan_preserved_messages` before the row is blanked, readable only through the audited `GET /workspaces/:id/legal-holds/preserved` (`token:admin`); lifting disposes of them. Without a hold a withdrawal deletes the earlier versions too. The hold itself is admin-only to see. Basis: Slack, Microsoft 365/Teams (`SubstrateHolds`), Google Vault and Mattermost all preserve deleted and edited content under a hold, hidden from the user and released at lift; FRCP 37(e); GDPR Art. 17(3)(e); Sedona Commentary on Legal Holds | M |
| ~~413.4a~~ | ~~**D-A:** governance and membership~~ **✅ 413.4a** — freeze, membership, review requirements and reviewers, land-gate clear, governance skills, egress targets, app revoke and secrets commit with their row on REST and MCP. MCP governance tools recorded nothing before; a concurrent write could let a non-admin lower a review requirement | M |
| ~~413.4b~~ | ~~**D-A:** SCIM users~~ **✅ 413.4b** — provisioning creates the member and its link together; deactivation and deletion revoke every live token, each recorded, in one transaction or fail (500, so the IdP retries). Before, a failed revoke was logged and the deprovision reported success with the token still live | S |
...
| Item | Verified state | Size |
|---|---|---|
| ~~**Helm digest pinning**~~ **✅ 415.1** | `image.digest` renders `repo@sha256:…`; the tag becomes informational | S |
...
| ~~#50 Goose claimant~~ **✅** | `examples/goose/maidan-waiter.yaml`: a Goose recipe running the waiter loop, one task per run, with the approval gate for anything irreversible. Maidan's side was verified by replaying its steps with a scoped app token; not run inside Goose, which needs an LLM provider. OpenHands is not written | S |
...
- **Legal-hold follow-ups** (after the preservation decision): (1) ~~**holds per matter**~~ **✅** — one hold per matter (`POST /workspaces/:id/legal-holds`, lift by id); the workspace is held while any stands, and only the last lift disposes; (2) **withdrawn bodies in the event log** — ~~member-facing~~ **✅** the events list reads a withdrawn message's posted and edited events back with the body blank; the whole-log tier (admins' catch-up, peers) still carries the words, so GDPR purge needs crypto-shredding there, a decision (L); (3) ~~delete a withdrawn message's embeddings~~ **✅** — withdrawing deletes them in the tombstone transaction, and the indexer embeds only a live message (Postgres takes the row `FOR SHARE`)
- ~~**The event log served every private channel and DM to any member**~~ **✅ fixed** (found 2026-09-25 while researching legal holds) — `GET /workspaces/:id/events`, its `/ui/api` twin, `/events/catch-up` and MCP `catch_up_events` checked only `workspace:read`, then returned every event with its body. The live streams filtered (#1029); the backfill never had. The events list now filters by the caller's thread and channel access (`event_visibility`) inside its paging loop; catch-up, the whole log as one chain, is `token:admin` or a peer. `event_log_channel_access_e2e` pins it with a private channel, a DM and three readers
...
- ~~**No way back from `in_review`**~~ **✅ request changes (David, 2026-09-25)** — a `request_changes` review from the owner or a counting reviewer sends an `in_review` thread back to `open`: claimable again, earlier approvals dismissed, the note in `get_thread_context` as `change_requests`, the reopen logged. `transition_thread` does not take it; the review is the only way
...
- **P1.1a — ✅ FIXED (Cluster 333):** MCP `edit_message` now uses `edit_message_with_event` + the new
  ...
  write-path parity) for the MESSAGE + SOCIAL tools is COMPLETE** — those mutations emit their domain
...
- **P1.1c — MCP assignment dual-write (the P0, home of the 209/351 write-path). ✅ FIXED (Cluster 374).**
  ...
  **DoD met:** `mcp_claim_next_reclaim_emits_claim_expired_then_assignment` (m1 claims a `lease_secs=-1`
  ...
  REST end to end (message, social, assignment). P1.1d (FSM transition) closed in Cluster 384.**
- **P1.1d — no MCP `transition_thread` twin. ✅ FIXED (Cluster 384).** Treated as a **gap**, not a
  forever land-gate. MCP `transition_thread` (`thread:transition`; `{thread_id, actor_id, action}`)
  calls `transition_thread_with_event` + `publish_stored` — identical SoD / required-reviewers
  close-gate / unresolved `refutes` / Cluster-383 critical composition as REST. Terminal transitions
  emit `ThreadReady` for newly-ready dependents. No bypass. **Don't be fooled by the capability
  name (verified 2026-09-12, still true):** other `thread:transition` tools (claim, owner, result,
  #22 review tools) never called `transition_thread_with_event`; shipping them did not close this.
  **DoD:** `transition_thread_tool_advances_fsm_and_honors_gates` (happy path + SoD denial +
  close-gate refusal). **⇒ MCP write-path parity is complete** (message, social, assignment, FSM).
- **P1.1 MCP write-path parity — the ORIGINAL finding (SUPERSEDED).** *(Kept for provenance. The
  ...
  MCP write tools (`cast_vote`/`add_reaction`/`remove_reaction`/`pin_message`/`unpin_message`/
  `edit_message`/`record_mention`/`add_reference`) call plain non-`*_with_event` store methods and append
  **no** domain event; `tools/message.rs::edit_message` calls `store.edit_message` (event-less) → an MCP
  edit appends no `MessageEdited`, so the flagship as-of replay returns the stale body forever and
  embeddings never reindex; MCP `post_message` never publishes `MentionRecorded`.~~
...
room** — external sandbox/eval and verifier live outside this repo. Do not become a harness,
...
- **Occupancy clocks — 190–192 ← G1 + H8 + H9 (Wave 1 #2, the standing hole). ✅ SHIPPED — Cluster 351
  (`v351.0.0`, PRs #620–#626), [[Retros/Cluster 351]].** What landed: `claim_lease_id` (a Burns *resource-version*
  fencing value) minted on claim/assign, enforced on `renew`/`acknowledge`/`release`; the `work_started_at` working
  clock stamped by `acknowledge_claim`; `GET /channels/:cid/occupancy` painting **queued ≠ claimed ≠ working ≠
  blocked**; `release_claim` (fenced graceful handoff); the reactive `ClaimExpired` event + `wait_for_claim_expired`.
  **Deviations from the original brief, on purpose:** (a) **`ClaimExpired` is LAZY** (fires when `claim_next`
  reclaims an expired lease), **not eager on process death** — a graceful SIGTERM/drain instead calls `release_claim`
  (which emits `ThreadAssignmentChanged`); an eager-on-death `ClaimExpired` would need a reaper/finalizer and is a
  **follow-up**, not built. (b) **No `ClaimUnacknowledged` event** — the occupancy view's `claimed` count (assigned,
  live lease, `work_started_at` NULL) surfaces "assigned, never acknowledged" by poll; a push event for it is a
  follow-up. (c) `ClaimFailed`/budget-stop is the T1/T5 row (#9), not built here. **Remaining thickeners (still
  open):** the TLA+/TLC (or reduced loom/madsim) two-clocks + fencing **invariant model** is a **G1 test** →
  **moves to Program V** (`NEW-stateful-proptest` / `NEW-loom-interleaving` / `NEW-madsim-deterministic` /
  `NEW-kani-critical-units`); PGMQ-shaped `visibility_timeout` (ack vs expire vs DLQ) + kube-rs finalizer/SIGTERM →
  eager-`ClaimExpired` drain recipes; the eager-death clock closes `NEW-conn-leak-detector`'s sibling risk (a
  drained replica holding a claim). Guardrails honored: didn't split the clocks, import a harness agent-loop, add a
  second journal table, or put N8 chrome on this row.
- **Held gate — 174 ← F3 + F9 + H2 + H10 + N6 (Wave 1 #1, the first NEW row). ✅ SHIPPED — Cluster 350
  (`v350.0.0`, PRs #612–#619), [[Retros/Cluster 350]].** What landed: `request_approval` returns an async
  `input_required` handle + an HMAC-verified `requestState`; three-way accept/decline/cancel; sampling/roots +
  the dead `request_client` subsystem deleted; gates **queryable** (`GET …/approval-gates`); empty accept is not
  yes + **silence is not consent** (enforced by the CAS `resolve` → 409); **required-human is a claim gate** (N6);
  `/ui` Approvals tab is the elicitation client; a Playwright `/ui` harness in CI. **Deviation, on purpose:** the
  brief said *drop* the `2024-11-05` protocol version — it was **KEPT** (an old-but-valid way to use the transport,
  not dead code; the maintainer's cleanup rule). **Remaining thickeners (still open):** **ext-auth step-up**
...
- **HITL list — 282–289 follow-up ← H12 (Wave 1 #3). ✅ SHIPPED — Cluster 352 (`v352.0.0`), PRs #628–#631**
  ...
  a2a-tck MUST failures as an external verifier `/test` (`NEW-a2a-tck-ci`).
...
- **SDK 0.2 typed DTOs** (all four SDKs return generic JSON today) + `NEW-sdk-ergonomics` (client idempotency keys
  ...
  tests against the official MCP TS/Python + `a2a-rs` SDKs are **an external verifier `/test`** (`NEW-mcp-inspector-ci` /
...
1. ~~**F3 + F9 + H2 + H10 (+ N6)** — the held gate on cluster **174**.~~ **✅ SHIPPED — Cluster 350 (`v350.0.0`), PRs #612–#619** ([[Retros/Cluster 350]]). Durable queryable approval gate (async `input_required`, HMAC `requestState`, CAS resolve, 409/400/403), N6 claim gate, sampling/roots + `request_client` retired, Playwright `/ui` harness, Approvals tab. **G1's `on_timeout`/escalation enum (G4) rides #13, after this gate — still open.**
2. ~~**G1 + H8 + H9** — occupancy clocks, EXPAND 190–192.~~ **✅ SHIPPED — Cluster 351 (`v351.0.0`), PRs #620–#626** ([[Retros/Cluster 351]]). Two clocks (`claim_lease_id` fencing + `work_started_at` working clock), `GET /channels/:cid/occupancy` (queued/claimed/working/blocked), `release_claim`, reactive `ClaimExpired` + `wait_for_claim_expired`. **Remaining KEEP — README + SDK 0.2 — splices here, after #2. The G1 formal two-clocks invariant model (TLA+/loom/madsim) was NOT built — it moves to Program V (verification as proof).**
3. ~~**H12** — the HITL list, EXPAND 282–289.~~ **✅ SHIPPED — Cluster 352 (`v352.0.0`), PRs #628–#631** ([[Retros/Cluster 352]]). A pending Cluster-350 held gate now surfaces as an `input-required` A2A task (synthetic `gate_as_task`, RBAC-checked), so an external agent can discover it via `tasks/list?status=input-required`; plus `status`/`statusTimestampAfter` filters, `pageSize` max 100, `application/a2a+json`, `includeArtifacts`. **Deferred: 352.5 real `nextPageToken` keyset paging** — needs the per-channel RBAC filter pushed into the store query (post-fetch filter breaks a cursor); the always-`""` token is already conformant for the ≤`pageSize` common case H12 hits.
4. ~~**N8 / B1 + N7**~~ **✅ SHIPPED — Cluster 353 (`v353.0.0`), PRs #633–#636** ([[Retros/Cluster 353]]). The vanilla `/ui` identity chrome, no SPA: **N8** — a **capability card** `{can, can't}` from `GET /me`'s real grant (353.1; `WhoAmI.known_capabilities`), per-thread **session-chrome badges** running/idle/needs-input/needs-approval/done (353.2), and **attenuation chrome** — minting cannot widen the caller's grant (353.3, the client mirror of `validate_subset`). **B1 + N7** — the tab bar is a WCAG-AA ARIA tablist (roving tabindex + Arrow/Home/End) with a skip link + `:focus-visible` (353.4); `lang="en"` was already present. Guarded by four `ui_js_contract` static checks + four Playwright specs. **Residual:** `NEW-ui-design-tokens` (a formal token system) + a full automated axe-core a11y sweep were not done — the pass was targeted (tablist/skip-link/focus), logged as a thickener.
5. ~~**H4** — lookback on `wait_for_*` + evict-on-wait; idempotent pre-wait side effects; no occupancy I/O in Drop.~~ **✅ SHIPPED — Cluster 354 (`v354.0.0`), PRs #638–#640** ([[Retros/Cluster 354]]). All five `wait_for_*` long-polls take an opt-in `since_log_id` lookback (subscribe-before-lookback = gapless, RBAC-filtered, reuses `list_events_after`), closing the drain/subscribe race. evict-on-wait + no-occupancy-I/O-in-Drop were already-holding invariants, now codified (a `PresenceRegistration::drop` comment; the agent-author contract in Integration.md — a wait doesn't renew the claim lease, so a waiter that outlives its lease is reclaimed; pre-wait side effects must be idempotent for resume).
6. ~~**W1 = G-dev-2 + G-dev-4** — a durable human owner ≠ claimer; the claimer cannot merge its own PR; persist steer; notify the owner on stuck.~~ **✅ SHIPPED — Cluster 355 (`v355.0.0`), PRs #642–#646** ([[Retros/Cluster 355]]). `Thread.owner_id` (the accountable party, orthogonal to the claim axis) + `set_thread_owner` REST/MCP; **separation of duties** enforced in the shared thread transition (an owner-governed thread's claimer cannot land its own work; inert until an owner is set); **persisted steer** (`maidan_thread_steer`, survives handoffs) over REST + MCP; **notify-owner-on-stuck** (a `ClaimExpired` on an owned thread → the owner via the notification router). Owner-governance is opt-in.
7. ~~**F1 + F2 + F7** — every post is a titled thread; collapsed children; leaf mute; `ThreadBumped`.~~ **✅ SHIPPED — Cluster 356 (`v356.0.0`), PRs #648–#652** ([[Retros/Cluster 356]]). A thread is now a first-class, titled, navigable object: **356.1** collapsed child threads (`ChildThreadSummary` + `GET /threads/:id/children`); **356.2** thread activity bump (a post bumps `updated_at` in-tx + `GET /channels/:cid/recent-threads`; **no `ThreadBumped` event** — `MessagePosted` already carries the `thread_id`, so the bump is a projection, not a new EventKind); **356.3** leaf mute (`maidan_thread_mutes` + `POST`/`DELETE /threads/:id/mute`, per-kind-independent, honored on both router paths); **356.4** rename thread (F1: `PUT /threads/:id/title` + MCP; a rename is metadata, so it does not bump the activity clock); **356.5** the threading MCP parity tools. **Deferred (stretch sub-item):** *Automerge as thread collab* (causal edits on the same titled thread) — **the event log stays the log**, do not CRDT the log; a separate, larger design, not folded here.
8. ~~**N3** — per-thread/channel mute + mention breakthrough + projector-kind overlay; **replace** the kind-only PK `(member, kind)` (241–243).~~ **✅ SHIPPED — Cluster 357 (`v357.0.0`), PRs #654–#657** ([[Retros/Cluster 357]]). Per-thread mute shipped in 356.3; this cluster added the **per-channel** mute (`maidan_channel_mutes`, 357.1) + **mention breakthrough** (357.2: a `MentionRecorded` pierces a channel mute — hierarchy kind > thread > channel) over REST + MCP (357.3). **Design call:** a per-scope side table (consistent with 356.3), NOT a migration of the kind-only `maidan_notification_prefs` PK — mute is now scopeable to kind/channel/thread. **Deferred (N3 sub-item):** the *projector-kind overlay* (muting by Slack/GitHub projector origin) needs the notification to carry its projector provenance — a separate design. Do not fold onto the projector-link row (346 / 349.4).
9. ~~**T1 / T5** — a tokens+USD+turns budget envelope that **stops the run**; an agent-work DLQ; hard stop ≠ success (`ClaimFailed`).~~ **✅ SHIPPED — Cluster 358 (`v358.0.0`), PRs #658–#662** ([[Retros/Cluster 358]]). A task/run carries a token/USD/turn/**wall-clock** envelope (`maidan_thread_budgets`, 358.1; USD as integer micros, wall from the 351 working clock); `report_usage` accumulates and, when a *claimed* thread goes over, atomically releases the claim + emits **`ClaimFailed`** (hard stop ≠ success; 358.2) + dead-letters the run to `maidan_agent_work_dlq` (358.3), over REST + MCP (358.4). **Design:** budget-exhaustion is a claim-level failure, NOT a new terminal `ThreadState`; enforcement is at the report heartbeat (no reaper — a silent hung agent is still caught by lease-expiry reclaim, 351); **not a billing SKU**. **Deferred (logged):** a reactive `wait_for_claim_failed` long-poll; wall-enforcement on the lease-reclaim path; a DLQ replay/retry action; OpenMeter entitlements → stop; the `NEW-assembly-slo` canon (named p99 + tail-amplification guard).
10. ~~**N2 / N5 / N4** — a digest of **buried decisions** (replace the unread-count digest, 254–257); an inbox grouped by thread + snooze; `during:` date-range on the existing search.~~ **✅ SHIPPED — Cluster 359 (`v359.0.0`), PRs #663–#667** ([[Retros/Cluster 359]]). **N4** date-range search (`SearchFilters {after, before}`, half-open window on `posted_at`, both backends × lexical+semantic, REST+MCP; 359.1); **N5** notification snooze (`snoozed_until`, drops out of inbox+badge, resurfaces on lapse; 359.2) + inbox **grouped by thread** (`group_notifications_by_thread`, pure fn; 359.3); **N2** the digest now **leads with buried decisions** — `ThreadResult`s a member missed in followed channels/threads since their last digest, over the sweeper + `GET /members/:id/decisions` + MCP (359.4). **Deferred:** a semantic date-range test, cross-page grouping, ~~a `result_kind=decision|plan|merge_authorized` facet (Open Work #24)~~ **✅ the facet shipped as Cluster 381** (namespaced-string list, not that enum, not message-FTS), a snoozed-item view. **N6 is not here** (it is the claim gate on #1).
11. ~~**G-dev-1** — a frozen, **token-budgeted** context pack.~~ **✅ SHIPPED — Cluster 360 (`v360.0.0`), PRs #668/#671/#673/#672** ([[Retros/Cluster 360]]). The scoped pack now caps itself by **tokens**, not just rows: `maidan_types::pack` (`estimate_tokens` `chars/4`, `PackElision`, `fold_messages_to_budget` — keep the opener + recent tail, fold the middle into an auditable `elision` marker, "Lost in the Middle") wired into `?token_budget=N` on the REST pack (360.2) + the MCP pack (360.3); plus **child grounds** (360.4) — a child thread's pack carries the parent's opening ask + latest decision (`ParentGrounding`; withheld for a cross-channel/DM/tombstoned parent). **Deferred (other G-dev-1 sub-items, logged):** pack-exclude-stale (drop stale threads from a workspace pack); file-mediated handoff; honor AGENTS.md/GEMINI.md/CLAUDE.md dirs + in-repo `llms.txt` as pack **input** (Codex 32 KiB cap); the pack **pushback** contract language (challenge the framing; uncertainty is ask/stuck, not land). Not Cluster 331; not a docs-platform product. (Backed by MAST / "Lost in the Middle" / MemGPT.)
12. ~~**G-dev-7** — inbound `pull_request.merged` → `ThreadLanded`.~~ **✅ SHIPPED — Cluster 361 (`v361.0.0`), PRs #675/#678/#680/#681** ([[Retros/Cluster 361]]). The GitHub projector now steals the **landed fact**: a merged PR linked to a thread (`action=closed` + `merged=true`) publishes a non-federatable `ThreadLanded` event (361.1–361.2, reusing `get_github_issue_link`; the FSM is **not** auto-transitioned), which the notification router fans to the thread's owner + followers (361.3) and an agent can await via the `wait_for_landed` MCP long-poll (361.4). **Deferred:** a `closed`-unmerged/`reopened` signal (only a merge is a land); GitHub egress stays comment-only (no "merged" comment back on a Maidan close); no GitHub-login→member mapping for "who merged it".
13. ~~**G2 / G4 / G3 / G11** — wait-edges + `on_timeout`; an escalation enum (G4 **after** the held gate in #1); fair dispatch + Unclaimable + hard WIP. Steal the Restate/Temporal promise/timer *shape* onto wait-edges; do not become the engine.~~ **✅ SHIPPED across four clusters (362–365): G11 hard WIP → Cluster 362 (`v362.0.0`), PRs #684/#686/#688** ([[Retros/Cluster 362]]) — a per-workspace cap on concurrent **live** claims per member (`maidan_wip_limits`; `claim_next` → null / `claim` → 409 at the cap; counts live claims not queued ghosts; REST + MCP). **+ G3 Unclaimable → Cluster 363 (`v363.0.0`), PRs #690/#692/#694/#696** ([[Retros/Cluster 363]]) — a thread parked from dispatch with a reason (`maidan_thread_unclaimable`; `claim_next` skips it via a SQL clause, explicit `claim` → 409, `QueueDepth` gained an `unclaimable` bucket; REST + MCP). **+ G2/G4 wait-edges + escalation → Cluster 364 (`v364.0.0`), PRs #698/#701/#704/#705/#706** ([[Retros/Cluster 364]]) — a durable per-thread wait timer (`maidan_thread_waits`; `wait_until` + an `EscalationPolicy` `on_timeout` of `Notify`/`Park`) fired by an opt-in sweeper (`claim_next_due_wait` atomic fire-once → `WaitTimedOut` event → the notification router reaches the thread owner; `Park` marks the thread unclaimable, reuse 363); `on_timeout` is reach + park only, never an invented decision; REST + MCP. **+ G3 fair dispatch → Cluster 365 (`v365.0.0`), PRs #708/#712/#713/#715** ([[Retros/Cluster 365]]) — a per-thread dispatch **priority with aging** (`maidan_thread_priorities`; `claim_next` orders by `priority + floor(age_seconds/3600)` DESC with a created_at tiebreak, so priority jumps the FIFO queue while a long-waiting task ages up and never starves; REST + MCP). **✅ #13 is now COMPLETE** (G11 WIP 362 + G3 Unclaimable 363 + G2/G4 wait-edges 364 + G3 fair dispatch 365).
14. ~~**N1, T6, H15, SCIM-as-OIDC-P3** (four bullets, not one cluster)~~ **✅ SHIPPED — Cluster 366 (`v366.0.0`), four independent PRs #717/#718/#719/#720** ([[Retros/Cluster 366]]). **T6 legal-hold** (`maidan_legal_holds`; held workspaces' events survive retention pruning, audit prune freezes, purge/erase → 409; REST + `/operator/legal-holds`). **H15 OTel feature-gate** (default-on cargo `otel` feature; `--no-default-features` compiles the OTLP/tonic stack out). **N1 web-push** (`maidan_push_subscriptions` + VAPID/RFC-8291 RustCrypto crypto; router delivers iff no live WS, prunes `410 Gone`; REST). **SCIM-as-OIDC-P3** (`/scim/v2/` Users create/read/list-filter/replace/patch/delete + ServiceProviderConfig; `maidan_scim_users`; deactivation revokes tokens; `token:admin`). **Deferred (P3 follow-ups):** legal-hold audit-selectivity (needs `workspace_id` on `maidan_audit`); a `/ui` web-push subscribe button + durable push-retry queue; SCIM Groups + userName/displayName rename (needs `update_member`) + complex filters. **H1 (AG-UI) is Wave 2 #17**, not this bucket.

### Wave 2 — humans inhabit the workplace loop (#15–28; Handoff rule 8 flips for the Work tab — vanilla `/ui`, still no SPA)

15. ~~**B2 + B11 + B3** (three bullets)~~ **✅ SHIPPED — Cluster 367 (`v367.0.0`), stacked PRs #722/#723/#724** ([[Retros/Cluster 367]]). **Work tab** (queue-depth 224 + occupancy 351, threads, result 234 + DAG deps 217, schedules 226 — observe-only human console). **Prefs console** (delivery mode 256 / email 250 / muted kinds 242 / channel+thread follows 245, self-only). **Looking glass** (events-by-kind / thread-by-id / artifact-by-sha / peers explorer). All vanilla `/ui`, off the OpenAPI/capability-map contract (the 251 precedent); `ui_js_wires_*` static guards + `work`/`glass` Playwright specs. **Deferred:** human claim-next (needs a session `thread:transition`), live-refreshing Work console, richer DAG viz, skills management in the Work tab.
16. ~~**G15 + G9** — a **waiting-on-you inbox**~~ **✅ SHIPPED — Cluster 368 (`v368.0.0`), stacked PRs #726/#727/#729** ([[Retros/Cluster 368]]). A member-centric aggregate — assigned non-terminal threads + the workspace's pending approval gates + unread mentions — oldest-waiting first, each aged against an SLA (overdue flag); not `@everyone`. Pure `assemble_waiting_inbox` (maidan-types) shared by REST `GET /members/:id/waiting?sla_secs=N` (composes 3 existing store reads, no new store code) + MCP `get_waiting_inbox` + a `/ui` "Waiting on you" section atop the Work tab. **Deferred:** an SLA-breach *notification* (the router could emit when an item goes overdue); per-source SLAs; a `wait_for_waiting` long-poll.
17. ~~**H1** — an **AG-UI door** on the existing WS/SSE~~ **✅ SHIPPED — Cluster 369 (`v369.0.0`), PRs #731 (types+mapping) + #733 (SSE)** ([[Retros/Cluster 369]]). A **thread is a run**: `AgUiEvent` + a pure `agui_events_for(&Event)` (`ThreadCreated`→`RUN_STARTED`, terminal `ThreadStateChanged`→`RUN_FINISHED`/non-terminal→`STEP_STARTED`, `ClaimFailed`→`RUN_ERROR`, `MessagePosted`→`TEXT_MESSAGE_*`+`TOOL_CALL_*`, `ThreadLanded`→`CUSTOM`), surfaced over `GET /agui/stream` (workspace/channel/thread scoped) reusing the `/mcp/stream` bus-subscribe + replay; SSE-standard resume (`Last-Event-ID`/`after_id`, per-frame source `id:`); per-event RBAC (`can_access_thread`/`can_access_channel`); off-contract (`event:subscribe` inline, like `/mcp/stream`). No CopilotKit. **Deferred (follow-ups):** the **input direction** — a UI POSTing AG-UI `RunAgentInput`/interrupts/`editedArgs` = full replace (REST covers the mutations today); richer `STEP_*` semantics (a step per FSM edge is coarse); an AG-UI-native `/ui` panel. F3 stays MCP MRTR; ACP (#51) stays skipped (AG-UI is the HITL door).
18. ~~**G8 + W5 + G-dev-9** — a **recipe / thread-type**~~ **✅ SHIPPED — Cluster 370 (`v370.0.0`), PRs #735/#736/#737/#738/#739** ([[Retros/Cluster 370]]). A reusable thread-type blueprint (`RecipeSpec`: params, definition of done, retry, inline DAG children); `instantiate_recipe` builds parent + DAG children + skills in one tx, freezing the recipe bytes into a `maidan_recipe_runs` snapshot (copy-on-fire); a `task_schedules.recipe_id` seeds a run per firing, emitting `ScheduleSkipped` (new non-federatable EventKind) when the prior run is still in flight. REST (`/workspaces/:wid/recipes` + `/instantiate`) + MCP (`create_recipe`/`list_recipes`/`instantiate_recipe`). DAG validated at rest (pure `validate`, acyclic via Kahn), trusted at instantiation; a task is a thread (reuses the FSM + Cluster-217 deps + Cluster-231 skills). **Not a recipe VM** — retry/DoD ride the snapshot unenforced. **Deferred (follow-ups):** sub-recipes as references to other recipes (recursive instantiation); param templating in child titles; schedule-level params; full JSON-Schema param validation (a validator dep); webhook/slash seeding (the schedule path demonstrates seeding); DoD/retry *enforcement* (that's #22 reviewers / #25 land_gate). 331 stays declined.
19. ~~**G19 + T3** — **secret-ref**~~ **✅ SHIPPED — Cluster 371 (`v371.0.0`), PRs #741/#742/#743/#744** ([[Retros/Cluster 371]]). A named secret whose value never enters the event log — the log carries a `secret://<name>` reference, `maidan_secrets` holds the AEAD-encrypted value (the Cluster-189 keyring), and it's materialized only transiently. `secret:read`/`secret:admin` caps; REST + MCP `resolve` ("a consumer fetches at exec"); an egress `SecretBroker` substitutes refs on a webhook delivery only for hosts on `MAIDAN_SECRET_EGRESS_ALLOWLIST` (a non-allowlisted host gets the literal ref, at send time so the plaintext never persists). The route/broker layer owns the key; the store is key-agnostic; the broker fails safe. **Deferred (follow-ups):** per-workspace egress allowlists (env-global today); the broker on other egress paths (A2A, automation deliveries); secret-ref resolution inside slash/automation templates; a secret audit trail.
...
23. ~~**G6 + G-dev-3 + W3** — a **spawn budget**~~ **✅ SHIPPED — Cluster 376 (`v376.0.0`), PRs #765/#768/#770/#771/#772/#774** ([[Retros/Cluster 376]]). A per-workspace cap on agent fan-out — `max_children` per parent claim, `max_depth` nesting, `max_tools` per thread — refused at the spawn as a 409 `SpawnRejected` (MCP `InvalidParams`) plus a `ThreadSpawnDenied` event naming the axis, the cap, what the claim already held, and who tried. Brooks/Amdahl/Two-Pizza: coordination cost is n(n−1)/2, so the room does not admit a further agent onto a late sequential claim. Each axis is **opt-in** (`null` = unlimited, the Cluster-362 WIP-limit shape) and settable over REST (`PUT`/`GET /workspaces/:id/spawn-budget`) + MCP (`set_spawn_budget`/`get_spawn_budget`); the gate lives in the **store** so REST, MCP, recipes and the scheduler share one implementation per backend. External fan-out is capped too: `maidan_github_issue_links(thread_id)` is UNIQUE, so a claim holds **at most one GitHub link** (a bijection with the existing `(repo, issue_number)` key). Store (376.1) → children+depth gate (376.2) → max-tools gate (376.3) → REST+MCP config (376.4) → GitHub-link cap (376.5) → `ThreadSpawnDenied` (376.6). **A budget, not a scheduler** — it refuses the next helper; it does not re-plan or kill a running agent (that is the Cluster-372 freeze). **Deferred (follow-ups):** per-channel / per-member budgets (workspace-wide today); a `SpawnAxis::GithubLink` if the link cap ever becomes configurable; a "fan-out remaining on this parent" read that doesn't require attempting a spawn; a `/ui` budget panel (the operator console doesn't surface the WIP limit either); routing a denial as a notification.
24. ~~**G7 + G-dev-10** — the `claim_next` pack includes in-channel **accepted decisions**, and `result_kind` is a search facet~~ **✅ SHIPPED — Cluster 382 pack (`v382.0.0`, PRs #792/#797/#796) + Cluster 381 facet (`v381.0.0`, PRs #798/#799/#801/#802)** ([[Retros/Cluster 382]], [[Retros/Cluster 381]]). **Pack (382):** live `get_thread_context` (REST + MCP) carries token-lean `accepted_decisions` teasers for closed/archived in-channel results so the next `claim_next` claimer sees what the channel already decided. Waiter envelopes (`schema = maidan.waiter.result/1`) appear only when `status` is `reviewed`; opaque JSON on a terminal thread is treated as accepted. `claim_next` itself still returns `Option<Thread>` — the pack is the claimer's read. Default on; `include_accepted_decisions=false` drops it; withheld on DM / as-of / workspace-nested packs. **Facet (381):** `result_kind` is a **namespaced string** (e.g. `example.review.result/1`), not a closed enum — the 2026-09-12 correction, now shipped. Indexed on `maidan_thread_results` (pg 0087 / sqlite 0086); listed over `GET /workspaces/:id/results?result_kind=` + MCP `list_thread_results` (`workspace:read`; exact match; omit = every accessible non-tombstoned result; private-channel rows dropped). This is a thread-result **list**, not `GET …/search`. The ADR convention `"kind": "decision"` is not a filter value. **Row #24 is closed.**
25. ~~**G-dev-6** — a **LandGate gate pointer** on the thread (`{kind:"land_gate", status:pass|fail, artifact_sha?}`): the FSM will not `closed` without a pass from a land-gate-skilled member ≠ the implementer, and a **green/amber/red** land-gate vocabulary so the FSM will not `closed` on **accepted nonsense** (amber = flags-then-still-engages is not a land). The room holds the pointer; an external verifier records pass/fail and may run an independent evaluation dataset. Not a CI product / a judge panel in the room.~~ **✅ SHIPPED — Cluster 383 composition (`v383.0.0`, PRs #809/#811/#814/#815) + Cluster 385 pointer (`v385.0.0`, PRs #819/#821/#826/#827/#831)** ([[Retros/Cluster 383]], [[Retros/Cluster 385]]). Composition: a reviewed `example.review.result/1` with any `critical` finding from a review-skilled producer writes Cluster-375 `request_changes` and arms `k=1` when unset. Pointer: a thread-scoped `{kind:"land_gate", status, artifact_sha?, land}` row; `closed` refuses unless a **green pass** from a `land_gate`-skilled member ≠ owner/assignee. Amber (flags-then-still-engages) is not a land. Fail is always red. No row is vacuous green (additive, Cluster 375 shape). Room holds the pointer; an external verifier records pass/fail. Not a CI product / a judge panel. **Row #25 is closed.**
26. ~~**H7** — a **capability ticket** for cross-org artifact/share (a 48h incident room + files); keep LocalArtifactStore + S3; steal the ticket *shape*, not a mesh.~~ **✅ SHIPPED — Cluster 405, PRs #952/#953/#954 + close record** ([[Clusters/Cluster 405]], [[Retros/Cluster 405]]). One ticket grants a time-boxed, read-only view of one channel and an exact allowlist of workspace-linked artifact SHAs. A `token:admin` issuer creates/lists/revokes over REST or MCP; the external recipient uses a separate header-only `ShareTicket` credential on four `GET /share/*` routes. The raw `maid_share_…` value is returned once; only its SHA-256 digest persists. Invalid, expired, and revoked tickets are indistinguishable; cross-channel and unallowlisted resources are hidden; liveness is rechecked before LocalFS/S3 reads. **No guest member, write authority, mesh, multi-workspace ticket, claims, or act-as-any extension. Row #26 is closed.**
27. ~~**G14 + W2** — a **blocked-reason enum** (`dag|gate|human|child|quota|unclaimable`); `claim_next` skips blocked; `BlockedResolved` unblocks. Distinct from DAG-children-must-be-terminal.~~ **✅ SHIPPED — Cluster 386 (`v386.0.0`), PRs #817/#820/#822/#825/#830** ([[Retros/Cluster 386]]). Closed `BlockedReason` (not a namespaced string). Side table `maidan_thread_blocks` (pg 0089 / sqlite 0088); presence = blocked. `claim_next` skips the row; explicit `claim` is 409. Clearing emits `BlockedResolved` (non-federatable). REST `PUT`/`GET`/`DELETE /threads/:id/block` + `GET /channels/:cid/blocked` + MCP twins. **Not** Cluster 218 DAG-children-must-be-terminal (`child` is an orchestrator-named wait). **Not** a replacement of Cluster 363's unclaimable park table (`unclaimable` here is vocabulary). **Deferred:** `wait_for_blocked_resolved`; a `/ui` block toggle.
28. ~~**G16 / G18 / H3** (three bullets) — **follow a member's occupancy**; a **manager digest** (channel result/gate/stuck counts — compose notifications, not analytics); **run lineage** (`parentRunId`) on a thread.~~ **✅ SHIPPED — Cluster 387 lineage (#818/#824/#828/#829) + Cluster 404 follows/digest (#943/#944/#949/#947/#948)** ([[Retros/Cluster 404]]). **Lineage:** accepts the producer's `run_id` rather than minting a parallel id; nested occupancy is attributed and F7 mute stays orthogonal. **Member occupancy:** a durable same-workspace follow edge; REST/MCP follow management; live ephemeral presence plus assigned non-terminal work filtered through the caller's thread access. Meaningful lifecycle events reach access-eligible followers through existing mute/dedup policy. **Manager digest:** REST/MCP/email share notification-derived unread per-channel `result`/`gate`/`stuck` counts; workspace gates use a null-channel bucket. No analytics ledger and no persisted presence. **Row #28 is closed.**

### Result delivery — the external last mile (Clusters 377–381; COMPLETE)

**Added 2026-09-12** from the producer-side feature request (deliver a waiter result to Slack/GitHub),
...
Maidan side: `crates/maidan-types/tests/fixtures/waiter_result_v1.json` (Cluster 379.2 contract
lock). The producer-side copy remains `waiter_result_v1.json` in the producer repo.

**Why it lives here and not in pi or land_gate:** Maidan already holds the connectors, the
channel/workspace links, identity, audit, the lease and the HITL gate. land_gate is temporary, so a
...
**Scope fence.** Maidan does not run seats, does not render reviews, and does
not become a CI product. Cluster 383 reads `findings[].severity` for the
close-gate adapter only (`critical` → Cluster-375 `request_changes`); it does
not judge finding bodies or invent a land vocabulary. GitHub review `event`
stays `COMMENT` (380). It delivers trusted bytes to blessed surfaces,
durably, once. Only `rendered` / `summary` / structured `findings` ever leave
— never raw seat output.

- ~~**Cluster 377 — durable projector egress** *(absorbs row #38; the foundation)*~~ **✅ SHIPPED
  (`v377.0.0`), PRs #777/#778/#779/#780** ([[Retros/Cluster 377]]). **377.1** the `maidan_egress_outbox`
  store foundation (pg 0082 / sqlite 0081, the `maidan_mail_outbox` shape; both backends, multi-replica
  claim, zero-blast-radius) — with the dedup index and the FK-less `source_log_id` that make an
  every-replica enqueue safe. **377.2** `egress_worker.rs` (lease 120 s, backoff 30 s→1 h, dead-letter at
  8) + the projectors enqueueing instead of posting inline; spawned only when a sender is configured, so
  an unconfigured deploy stays inert. **377.3** retry-then-disable (`disabled_at`, pg 0083 / sqlite 0082)
  + `ProjectorMisconfigured` + `maidan_egress_deliveries_total{surface,outcome}`; the predicate is a
  per-surface allowlist with a rate-limit carve-out (GitHub's secondary rate limit is a 403). **377.4**
  `GET /operator/egress/dead` + requeue (`token:admin`, the Cluster-306 shape) — split out of the planned
  377.3 because an operator HTTP surface with a full new-route preflight is its own concern. The
  `(surface, selector)` pair the outbox persists is deliberately the pair Cluster 378's allowlist keys on.
- ~~**Cluster 378 — the trust boundary + sender upgrade.**~~ **✅ SHIPPED (`v378.0.0`), PRs
  #782/#783/#784** ([[Retros/Cluster 378]]). **378.1** `maidan_egress_targets` (pg 0084 / sqlite 0083) +
  store + a `token:admin` REST surface (`POST`/`GET /workspaces/:wid/egress-targets` + `DELETE …/:tid`,
  audited, full new-route preflight). **`deliver_to` selects, the allowlist authorizes**; default empty ⇒
  deliver nowhere. A selector must be an **id** (a mutable `#name` is not an allowlist key), and on GitHub
  the authorization grain is the **repository** — one blessing per repo, not an operator ticket per PR, via
  `EgressTarget::allowlist_selector()`. Reads are `token:admin` too: the allowlist is *policy*, and
  enumerating it would hand an agent the list of destinations worth aiming at. **378.2** the sender
  upgrade — `ExternalRef` from both `post_*` + `update_message` (`chat.update`) / `update_comment`
  (`PATCH …/issues/comments/{id}`) + `thread_ts`, extending `egress_wire_e2e` (347) from 5 to 13 tests
  over the real clients. **A post that succeeded is never reported as a failure** (`Ok(None)` =
  posted-but-not-addressable; an `Err` on the at-least-once queue would leave two comments). **378.3**
  `egress_body.rs` — pure mention neutralization (a GitHub mention wrapped in a **code span**, a
...
- ~~**Cluster 379 — the result-delivery primitive** *(the producer's actual ask)*.~~ **✅ SHIPPED
  ...
  `crates/maidan-types/tests/fixtures/waiter_result_v1.json`. **379.3** the `ThreadResultSet` arm:
  ...
  **Note back to the producer: the grammar is frozen at `maidan.waiter.result/1`.** Additive fields are
...
- ~~**Cluster 380 — inline per-finding PR review comments.**~~ **✅ SHIPPED (`v380.0.0`), PRs #803/#805/#806** ([[Retros/Cluster 380]]).
  ...
  **`line_range` frame (pinned 380.1):** file-absolute **post-image** lines at
  ...
  Cluster 379's summary comment path is unchanged. **Deferred:** last-posted-sha
  so a same-SHA replay no-ops the review; splitting >100 findings (declined).
- ~~**Cluster 381 — `result_kind` facet + the pinned spec** *(facet half of row #24; the pack half shipped as Cluster 382)*~~ **✅ SHIPPED (`v381.0.0`), PRs #798/#799/#801/#802** ([[Retros/Cluster 381]]). **381.1** `result_kind_from_payload` + indexed column on `maidan_thread_results` (pg 0087 / sqlite 0086) + `list_thread_results` both backends (382's `list_channel_closed_results` untouched). **381.2** `GET /workspaces/:id/results?result_kind=` (`workspace:read`; `can_access_thread` post-filter; full new-route preflight). **381.3** MCP `list_thread_results` (workspace from the token). **381.4** Integration + Result Delivery — discoverability is the namespaced string; `"kind": "decision"` is not a filter value (**not a retro**). Facet on the **namespaced string** (exact match; never a closed enum; not message-FTS). **Row #24 is closed.** `maidan.waiter.result/1` is registered in the Cluster 390 lexicon pack (`contracts/lexicon/`).
- ~~**Cluster 383 — critical → `request_changes` adapter** *(row #25 composition; not a new gate)*~~ **✅ SHIPPED (`v383.0.0`), PRs #809/#811/#814/#815** ([[Retros/Cluster 383]]). A reviewed `example.review.result/1` with any `critical` finding from a review-skilled producer writes Cluster-375 `request_changes` and arms `k=1` when unset. Empty `deliver_to` still arms. Never auto-approve. GitHub `event` stays `COMMENT`. LandGate pointer + green/amber/red shipped as Cluster 385.
- ~~**Cluster 385 — LandGate gate pointer + green/amber/red land vocabulary** *(row #25 remainder)*~~ **✅ SHIPPED (`v385.0.0`), PRs #819/#821/#826/#827/#831** ([[Retros/Cluster 385]]). Opt-in close-gate (no row = vacuous green). Qualifying land = green pass from a `land_gate`-skilled member ≠ owner/assignee. Amber is not a land. Fail is always red. Room holds the pointer; an external verifier records pass/fail. Cluster 384 is P1.1d. **Row #25 is closed.**
- ~~**Cluster 386 — blocked-reason enum** *(row #27)*~~ **✅ SHIPPED (`v386.0.0`), PRs #817/#820/#822/#825/#830** ([[Retros/Cluster 386]]). Closed `BlockedReason`; `maidan_thread_blocks` pg 0089 / sqlite 0088; `claim_next` skips; `BlockedResolved` on clear. **Row #27 is closed.** Follow-occupancy and the manager digest stay on #28.
...
enum corrected to a namespaced string **and shipped as Cluster 381**; row #28's `parentRunId` **lineage half shipped as Cluster 387** (pi's
`run_id` is the first real producer — accepted as-is, not reminted; follow-occupancy + manager digest remain); row #27 shipped as Cluster 386; row #25 shipped as Cluster 383 (composition) + Cluster 385 (pointer + land vocabulary). **Plus one that has no existing row:** pi's
...
29. ~~**B5 + B15 + H5** — subscribe `CursorTooOld` (fail loud, no silent clamp) + a durable `consumer_id` cursor + a thick SDK helper (HTTP backfill then WS cutover); projector **shapes** `{workspace, channel?, thread?, types[]}` + a 409 must-refetch; `RecvError::Lagged(n)` = resume-from-log + a metric, never a silent drop (Postel anti-pattern: fail loud).~~ **✅ SHIPPED (`v388.0.0`), PRs #832/#834/#835/#836/#837** ([[Retros/Cluster 388]]). 409 `must_refetch` on REST / WS / MCP SSE; durable cursor is existing `maidan_delivery_cursor`; `follow` HTTP-then-WS; `BusItem::Lagged` resumes from the log (`maidan_bus_lag_resume_total`). **Row #29 is closed.**
30. ~~**B6 + B21 + B17** — an **EventKind JSON-Schema pack** (lexicon analogue, feeds SDK 0.2); `$type` evolution (new fields optional, no renames, unknown ignored, breaking = new type — **Hyrum home**: observable `$type` is the contract); a `Maidan-Room-LSN` header on REST/WS/MCP/A2A/projectors so clients see projector lag (a broadcast-lag companion to the existing read-your-writes `Maidan-Consistency-Token`, v263 — do not conflate the two). Canon `NEW-snapshot-tests` over the normalized shapes.~~ **✅ SHIPPED (`v390.0.0`), PRs #839/#840/#841/#842/#843** ([[Retros/Cluster 390]]). Lexicon pack under `contracts/lexicon/` (`maidan.event.{kind}/1` + `maidan.waiter.result/1` + `example.review.result/1` / `example.plan.result/1`). `$type` on live WS/MCP frames and webhook bodies. `Maidan-Room-LSN` is the event-log high-water (decimal; always on, including SQLite); `Maidan-Consistency-Token` stays the replica WAL token. Four SDKs capture `last_room_lsn` at 0.1.0. **Row #30 is closed.** Do not start #31–36 from this close.
31. ~~**B7** — a **signed workspace export** a blank GHCR instance can verify without the origin host; document token continuity or an explicit "tokens die on export."~~ **✅ SHIPPED (`v391.0.0`), PRs #844/#845/#846/#847** ([[Retros/Cluster 391]]). Envelope `$type` `maidan.workspace.export/1`, Ed25519 operator key (`MAIDAN_EXPORT_SIGNING_KEY`), optional `MAIDAN_EXPORT_VERIFY_KEYS` pin. A blank instance verifies the file without calling the origin (embedded public key + canonical statement hash). **Tokens die on export** — secrets omitted; stuffed credential fields fail closed; mint new tokens after import. REST `GET /workspaces/:id/export` + `POST /workspaces/export/verify` + signed `POST /workspaces/import`; MCP twins. **Row #31 is closed.** Do not start #32–36 from this close.
32. ~~**B16** — a **hash-chained log + strong refs** (every event `{id, lsn, prev_hash, content_hash}`; `claim_next` + A2A citations pin `{uri, content_hash}`); a federated peer detects a break without trusting the host. Not MST/CAR.~~ **✅ SHIPPED (`v392.0.0`), PRs #849/#850/#851/#852/#853** ([[Retros/Cluster 392]]). SHA-256 (`sha256:<hex>`); hashed not signed; `lsn` = event-log `id` (not WAL); per-workspace chain; verify retained suffix (`GET /workspaces/:wid/events/verify`, 409 fail-closed); federation checks origin hashes before remap; `claim_next` pin + A2A `citations`. **Row #32 is closed.** Do not start #33–36 from this close.
33. ~~**B18 + B19** — a **snapshot + since-LSN catch-up** (getRepo-shaped) + a **tap projector contract** (verify, backfill, filter, live-waits-for-history, webhook/WS); search is a projector that must not diverge silently.~~ **✅ SHIPPED (`v393.0.0`), PRs #854/#855/#856/#857/#858** ([[Retros/Cluster 393]]). `$type` `maidan.event-log.snapshot/1` + `maidan.event-log.catch-up/1`. A peer takes a hashed domain-graph checkpoint at the retained floor and walks the Cluster 392 chain from that LSN. Header + `graph_hash` is `workspace:read` / peer; `include_graph=true` is `token:admin` or a registered peer. Pruned-gap cursors stay 409 `must_refetch` with a `snapshot` href — never clamp. Search is a tap: verify every backfill row (per-workspace chain), filter `SEARCH_PROJECTOR_KINDS`, drain history before live, `Lagged` without a log is `RebuildRequired`. Complements 392 (retained suffix); this row covers the pruned prefix. **Row #33 is closed.** Do not start #34–36 from this close.
34. ~~**B8 + B27** — a tombstone/deletion explorer + a backlink index ("what points at this message") + a kind census.~~ **✅ SHIPPED (`v394.0.0`), PRs #859/#860/#861/#862** ([[Retros/Cluster 394]]). Tombstones (`GET /workspaces/:id/tombstones` + MCP `list_tombstones`; `include_purged` reconstructs hard deletes from `MessageTombstoned`). Backlinks (`GET /messages/:id/backlinks` + MCP `list_message_backlinks` — Cluster 320 reverse edges + pins/reactions/votes; mentions outgoing, omitted). Kind census (`GET /workspaces/:id/kind-census` + MCP `get_kind_census`; `private_channel_deny_set` in the query). No new table. **Row #34 is closed.**
35. ~~**B20 + B22** — **named capability sets** (`maidan.agent.worker`, `maidan.human.admin`) + progressive grant + holder-side attenuation (`NEW-cap-attenuation` — Levy/Madden, not a Cedar rewrite); **stable `maidan://` URIs** (`maidan://{room}/channels/…/messages/…` optional `#content_hash`); optional `/.well-known/maidan-room`; a handle rename must not break stored IDs.~~ **✅ SHIPPED (`v395.0.0`), PRs #863/#864/#865/#866/#867** ([[Retros/Cluster 395]]). Sets are mint-time recipes (atomics still stored). Issuer mint accepts `capability_set`; `POST /tokens/attenuate` is holder-side (no `token:admin`). Room URI authority is the workspace UUID; handle is a sidecar alias. `GET /.well-known/maidan-room` is scheme-only. **Row #35 is closed.** Do **not** start #36 from this close.
36. ~~**B10** — a **WASI slash-handler kind** (a workspace installs a no-network guest with gas/memory caps; Maidan stores + invokes; the guest is the tool).~~ **✅ SHIPPED (`v399.0.0`), PRs #894/#895/#897/#898.** `wasmi` interpreter; 16-call preview-1 allowlist; fuel, memory, sandbox-output, and room-output bounds; workspace artifact ownership checked at registration and dispatch; explicit failure kinds. Not an agent runtime (that remains external). See [WASI handlers](WASI-Handlers.md). **Row #36 and Wave 3 are closed.**

### Wave 4 — stranger-start + ops leftovers (#37–51)

37. ~~**B12 + B14** — a **GHCR two-image boot smoke** (server `/health` + CLI `maidan init` against a throwaway DB) + a **loopback OIDC** (not `MAIDAN_OIDC_MOCK`).~~ **✅ SHIPPED (`v406.0.0`), PRs #957/#961/#959 + close record** ([[Retros/Cluster 406]]). A separate non-root multi-arch CLI image is built from the exact release archive, versioned with the release tag, signed, and scanned beside the server image. GitHub Release creation waits for a pull-by-tag consumer smoke of CLI init → fresh Postgres → exact-version server health → bearer `/me` success and anonymous rejection. A real loopback IdP drives production discovery, authorization code + S256 PKCE, token, ES256 JWKS, session, and logout paths; bad state/nonce/signature/audience/issuer fail without a cookie. **Row #37 is closed.**
38. ~~**N1 thicken** — **webhook retry-then-disable** on projector egress + a loud `ProjectorMisconfigured`/DLQ, never silent drop~~ **✅ SHIPPED — Cluster 377 (`v377.0.0`), PRs #777/#778/#779/#780** ([[Retros/Cluster 377]]). Projector egress was best-effort log-and-drop; it is now a durable queue. `maidan_egress_outbox` (pg 0082 / sqlite 0081, the `maidan_mail_outbox` shape) + `EgressStore` on both backends (377.1) → an `egress_worker.rs` with lease/backoff/dead-letter that the projectors **enqueue** onto instead of posting inline (377.2) → retry-then-disable: an auth/config-class failure sets `disabled_at` on the link (pg 0083 / sqlite 0082), the enqueue then skips it, and `Event::ProjectorMisconfigured` + `maidan_egress_deliveries_total{surface,outcome}` say so — with an explicit **rate-limit carve-out**, because GitHub answers a secondary rate limit with the same 403 as a revoked token (377.3) → `GET /operator/egress/dead` + `POST …/{id}/requeue`, `token:admin`, the Cluster-306 mail-DLQ shape (377.4). Re-linking clears `disabled_at` (no new route); **ingress is untouched** by an outbound credential failure. This was the hard prerequisite for result delivery, and it was a **reorder, not new scope**. **Deferred (follow-ups):** a `/ui` DLQ panel (the Cluster-137 operator tab); bulk requeue; a DLQ-depth alert rule with the SLO rules; an automatic re-enable probe; a projector-*wide* disable (today each link disables itself as its own delivery fails).
...
- `a2a-tck-ci` — run the official A2A TCK MUST suite in CI. `mcp-inspector-ci` — scriptable Inspector/official-SDK
  oracle over the full MCP tool catalog. `sdk-interop-oracle` — run the official third-party SDKs as clients. `openapi-lint`
...
- `erasure-cascade` — **GDPR Art 17 is unsatisfiable end-to-end today**: content-addressed dedup means a naive
  ...
  (scheduled re-encrypt + key-age policy atop the v189 read-side keyring). ~~`denial-audit`~~ **shipped in Cluster 410**:
  a bounded/sampled 401/403 signal + spike alert — the v182 write-amplifier deferral, done as an aggregate not a
  per-request write.
...
(a typed-reference explorer over the `RelationKind` edges + the Cluster 394 backlink index — the ontology made
navigable in the `/ui`; the index shipped, the pane has not), `docs-diagrams`
(mermaid), `openapi-reference-ui` (a Scalar-style interactive reference over the bijection-gated `/openapi.json`),
`demo-recordings` (asciinema/vhs demo-as-code), ~~`waiter-loop-recipe` (a consolidated agent-author *waiter loop* in `docs/Integration.md`: the full claim lifecycle `claim_next_thread → acknowledge_claim → get_thread_context → report_usage → set_thread_result → release_claim`, with an optional `request_approval` gate mid-loop — extending the six-tool hero loop in `examples/README.md` (frozen at Cluster 317) with the occupancy clocks (Cluster 351) and budget envelope (Cluster 358) it predates, so a serious waiter knows to *acknowledge* (so occupancy shows `working` not idle), *report usage* (budget can hard-stop → `ClaimFailed`), and *release* on graceful drain; plus the honesty note that **`ClaimExpired` is lazy** — fired only by `claim_next` reclaiming an expired lease, never eagerly on process death, so a draining agent should `release_claim` rather than let the lease lapse. Doc-only, no new tool/route/EventKind; neighbours the Cluster-354 "Long-poll waits" contract. Surfaced by the 2026-09 Pi-integration-prep review, which otherwise confirmed Wave 1/2 are honestly closed — no parallel "Pi prep" backlog)~~ **✅ SHIPPED (2026-09-12, docs-only)** — the "The waiter loop" section of [Integration](Integration.md) walks all six steps with capabilities and REST twins, and carries the lazy-expiry note, the "no client wall-clock argument" note (wall time is derived server-side from `work_started_at` against `max_wall_secs` when you `report_usage`), and the claim-gate/lease interaction on `request_approval`. Landed alongside the MCP transport honesty pass: `initialize.instructions` and the `examples/README.md` hero now name the acknowledge/report/release steps too.

**Program U addition — `docs-drift-guard` (found 2026-09-12 while shipping `waiter-loop-recipe`).** Two generated
facts about the MCP surface drift silently because nothing checks them:

- `book/src/mcp-reference.md` is tracked but the `docs` job regenerates it before every build, so the committed copy
  is never refreshed by hand. It had been untouched since Cluster **118** (2026-06-18) and was **3,023 lines** behind
  the live catalog — missing `whoami` and ~54 other tools, and still advertising "MCP 2024-11-05 subset" as *the*
  transport. Regenerated in the honesty pass, but the fix is structural: either gitignore it the way
  `book/src/docs/` already is (a local build already needs `book/sync-docs.sh` first, so requiring the generator too
  is consistent) or add a CI step that regenerates and fails on a diff.
- The tool **count** in prose re-drifts every cluster. #766 dropped the literal from three integrator pages rather
  than update it again; the durable form is a check that no doc states a count disagreeing with
  `contracts/mcp-tool-names.json`. Same guard, same job.
...
**Anti-goals & honesty gates (carried from the round):** no harness/sandbox/eval-gauntlet (those live outside this room);
...
room*. Folded here. **Lane tags:** `generic-room` (any external waiter), `oss-adoption`
(stars/`/play`/registries), `first-consumer` (first-consumer work lives with the consumer, not a Maidan
cluster). **Sequence:** correctness first (315) → honesty/no-clone (316) → snippet pack
...
| **F-56** | **shipped — #973** | Remaining MinIO and mc image inputs are digest-pinned |
...
Executed as Cluster 408 under umbrella #971:

1. **A — self-scoping + input bounds.** Shipped in #964.
2. **B — egress + deploy hardening.** Shipped in #973.
3. **C — `/ui` P1.** Shipped in #974.
...
| Unbounded grant lifetime | **Real.** `validate_new` only requires `expires_at > now` | **Decided D-B: per-workspace ceiling, default 90 days. Shipped in 413.1** |
...
- **Prior tag:** **`v349.0.0`** (`0728ead`, post-gate hardening, Phase XXIV). Since v273: MCP `2026-07-28` (300–303), durable mail retry (304–306), Slack + GitHub projectors (307–312), the SDK arc published at 0.1.0 (294–299), launch-prep (313–314), the 2026-08-28 sweep (315–318) + the fidelity/context flagship arc (319–331), and the post-flagship audit program (332–349). *(Narrative below is the historical v273 program record.)* Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189); **Arc C (agentic task-queue depth) COMPLETE** (190–197). **Arc D — performance & scale**: tractable perf wins DONE — 198 load/soak harness (`scripts/loadgen.sh` + `#[ignore]`d `load_baseline`), 199 concurrent workspace-context assembly (bounded `buffered` per-thread builds), 200 filtered-ANN search (RBAC private-channel deny pushed into the query; honors `limit`, no leak), 201 workspace-sharded event fan-out (`ShardedBroadcast`; O(relevant) not O(all)). **Arc D remaining items — assessed + deferred, NOT abandoned:** *(Narrative below is the historical v273 program record.)* Post-v155 four-arc program complete (156–178). **Security-led four-arc program: Arc A (security & correctness) COMPLETE** (179–184); **Arc B (multi-tenant SaaS ops) COMPLETE** (185–189); **Arc C (agentic task-queue depth) COMPLETE** (190–197). **Arc D — performance & scale**: tractable perf wins DONE — 198 load/soak harness (`scripts/loadgen.sh` + `#[ignore]`d `load_baseline`), 199 concurrent workspace-context assembly (bounded `buffered` per-thread builds), 200 filtered-ANN search (RBAC private-channel deny pushed into the query; honors `limit`, no leak), 201 workspace-sharded event fan-out (`ShardedBroadcast`; O(relevant) not O(all)). **Arc D remaining items — assessed + deferred, NOT abandoned:**
...
- **Assignment queue follow-ups (Clusters 190–192, extended by Cluster 351):** MCP tools shipped in 191; claim leases + reclaim shipped in 192. **Cluster 351 added** claim fencing (`claim_lease_id`), the working clock (`acknowledge_claim`), the occupancy view, `release_claim` (a dead/draining holder now *actively* returns its work), and the reactive `ClaimExpired` event (a reclaim now **emits an event** naming the dead holder). **Remaining:** `claim_next` is still channel-scoped (no workspace-wide pull); still no server-side default lease (the caller sets `lease_secs`); reclaim is still **lazy** (an expired lease is freed on the next `claim_next`, and `ClaimExpired` fires then — nothing *eagerly* reaps a dead holder on process death unless it calls `release_claim`; the eager-on-death path + a `ClaimUnacknowledged` push event are follow-ups, see the occupancy EXPAND row).

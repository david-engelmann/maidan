...
24. ~~**G7 + G-dev-10** — the `claim_next` pack includes in-channel **accepted decisions**~~ **✅ PACK HALF SHIPPED — Cluster 382 (`v382.0.0`), PRs #792/#797/#796** ([[Retros/Cluster 382]]). Live `get_thread_context` (REST + MCP) carries token-lean `accepted_decisions` teasers for closed/archived in-channel results so the next `claim_next` claimer sees what the channel already decided. Waiter envelopes (`schema = pi.waiter.result/1`) appear only when `status` is `reviewed`; opaque JSON on a terminal thread is treated as accepted; `result_kind` is a **namespaced string** (e.g. `pi.review.result/1`), not a closed enum. `claim_next` itself still returns `Option<Thread>` — the pack is the claimer's read. Default on; `include_accepted_decisions=false` drops it; withheld on DM / as-of / workspace-nested packs. **⚠️ CORRECTION (2026-09-12) stands:** do not facet on a closed enum. **Remaining half of this row:** facet `result_kind` into the existing search — **Cluster 381** (open; 381.1–381.2 on `main`).
...
### Result delivery — the external last mile (Clusters 377–381; 377–380 shipped, 381 is the remaining `result_kind` facet of row #24)

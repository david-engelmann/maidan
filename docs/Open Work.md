...
8. **N3** — per-thread/channel mute + mention breakthrough + projector-kind overlay; **replace** the kind-only PK `(member, kind)` (241–243). Do not fold onto the projector-link row (346 / 349.4). **← NEXT (the current next-ranked feature).** Note: Cluster 356.3 already delivered the per-*thread* mute as a side table (`maidan_thread_mutes`); N3's remaining scope is the prefs-PK replacement, a per-*channel* mute, and a **mention breakthrough** (a mention pierces a channel mute).
...
- **Latest merged: Cluster 356 (the threading cluster) on `main`; tags `v350.0.0`–`v356.0.0` pending the maintainer.** The forward program's **Wave 1 #1 (held gate, 350), #2 (occupancy clocks, 351), #3 (HITL list, 352), #4 (identity chrome, 353), #5 (wait contract, 354), #6 (owner/steer, 355), and #7 (threading, Cluster 356, PRs #648–#652) are all SHIPPED** ([[Retros/Cluster 350]], [[Retros/Cluster 351]], [[Retros/Cluster 352]], [[Retros/Cluster 353]], [[Retros/Cluster 354]], [[Retros/Cluster 355]], [[Retros/Cluster 356]]). A thread is now a first-class, titled, navigable object: collapsed children, an activity bump, leaf mute, rename — over REST + MCP. **Next up: Wave 1 #8 — N3** (per-thread/channel mute + mention breakthrough + projector-kind overlay; **replace** the kind-only notification-prefs PK; note 356.3's leaf mute already delivered the per-thread half). *(v350–v356 tags not yet cut — a `git tag` triggers `release.yml` image builds; left for the maintainer.)*

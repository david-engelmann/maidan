...
28. **G16 / G18 / H3** (three bullets) — **follow a member's occupancy**; a **manager digest** (channel result/gate/stuck counts — compose notifications, not analytics); ~~**run lineage** (`parentRunId`) on a thread~~ **✅ SHIPPED lineage half — Cluster 387, PRs #818/#824/#828/#829** (nested occupancy attributed; F7 mute stays orthogonal). Accepts the producer's `run_id` (waiter fixture `aa4dc966-0e09-44c3-b7a5-2d048b48b301`) rather than minting a parallel id. `maidan_thread_lineage` (pg 0090 / sqlite 0089); REST `PUT`/`GET`/`DELETE /threads/:id/lineage` + workspace `run-threads` / `run-occupancy`; MCP `set`/`get_thread_lineage` / `list_run_threads` / `get_run_occupancy`; auto-home on `set_thread_result`. Delivery parse still ignores `run_id`. **Remaining on this row:** follow a member's occupancy; the manager digest. **Do not claim the whole row closed.**

...
| Change | Where |
|--------|-------|
| **Self-scoped personal state:** REST and MCP use authenticated acting identity for email, preferences, inbox state, follows, and other personal operations. Cross-member use requires the workspace-bound `member:impersonate`, whose use is logged via `tracing` rather than written to the durable audit trail; work attribution stays orchestrator-controlled pending the Cluster 410 delegation work. Confirmed client limits are clamped before SQL. | `crates/maidan-{auth,mcp,server}/`, `contracts/http-capability-map.json` |

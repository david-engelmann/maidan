...
#[tokio::test]
async fn get_set_member_delivery_mode() {
    ...
    // Default (never set) is immediate.
    let got: Value = client
        .get(format!("{base}/members/{mid}/delivery-mode"))
        ...
        .await
        .unwrap();
    assert_eq!(got["mode"], "immediate");

    // Switch to digest.
    let set = client
        .put(format!("{base}/members/{mid}/delivery-mode"))
        .header("Authorization", &bearer)
        .json(&json!({ "mode": "digest" }))
        .send()
        .await
        .unwrap();
    assert_eq!(set.status(), StatusCode::OK);
    let body: Value = set.json().await.unwrap();
    assert_eq!(body["mode"], "digest");
    ...
    let got: Value = client
        ...
        .await
        .unwrap();
    assert_eq!(got["mode"], "digest");

    // An unknown mode is rejected at the extractor.
    let bad = client
        .put(format!("{base}/members/{mid}/delivery-mode"))
        .header("Authorization", &bearer)
        .json(&json!({ "mode": "carrier-pigeon" }))
        .send()
        .await
        .unwrap();

//! Projector / broadcast lag token.
//!
//! Stamps `Maidan-Room-LSN` with **the caller's room** high-water so a client
//! can compare its last-seen `log_id` to the head it is actually chasing.
//!
//! Scoped per room. It previously reported the instance-wide `MAX(id)`,
//! which made the number incomparable to anything a client had seen: a fully
//! caught-up projector could never reach it, because the remaining gap was
//! other tenants' writes. It also handed every tenant the instance's total
...
/// through the **response** extensions.
///
...
/// The head of **one room's** log.
///
/// The header answers "how far behind is my projector?", which only works if
/// the number is comparable to a `log_id` the client has actually seen — and a
/// client only ever sees its own workspace's events. Reporting the
/// instance-wide head meant a fully caught-up projector could never reach it,
/// because the gap was other tenants' writes; it also told every tenant the
...
pub async fn middleware(State(state): State<AppState>, req: Request, next: Next) -> Response {
    ...
    // Scoped to the caller's room. No scope means the response was not produced
    // for an authenticated room, so there is no head to report — which also
    // keeps the header off pre-auth responses.

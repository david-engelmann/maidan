...
#[openapi(
    info(
        ...
        description = "The operating layer for teams of AI agents. A team of agents coordinates work, keeps a durable searchable record, and pulls exactly the context each step needs, so they do better work for fewer tokens. Human login uses OIDC + `maidan_session` cookie (see `auth` tag).\n\n\
            ...
            MCP streamable HTTP POST /mcp/streamable: default/negotiate MCP `2026-07-28` (Clusters 300–303) — stateless Streamable HTTP; a modern client POSTs cold (no session required) and gets a single JSON-RPC response. When present, SEP-2243 `Mcp-Method` / `Mcp-Name` routing headers must match the JSON-RPC body (else 400). `2024-11-05` is still accepted on explicit request; that era keeps streamable session (`Mcp-Session-Id`) + `Last-Event-ID` resumability, but it is not the only path. A client that sends `Accept: application/json` (no `text/event-stream`) gets a single JSON body on either era. GET /mcp/streamable opens a server→client SSE stream (unsolicited notifications) on the 2024 session path. Supports JSON-RPC batches and notifications (202); `MCP-Protocol-Version` header validated. Human approval is the durable held gate (Cluster 350): MCP `request_approval` returns `input_required` immediately; humans answer over REST/`/ui`. The server does not issue sampling / roots / elicitation over a session back-channel (that path was removed).\n\n\
    ...
    components(schemas(
        ...
        maidan_types::BudgetLimits,
        maidan_types::UsageDelta,
        maidan_types::UsageReport,

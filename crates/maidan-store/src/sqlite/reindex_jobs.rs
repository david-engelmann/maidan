...
pub async fn upsert(pool: &SqlitePool, job: ReindexJob) -> Result<(), StoreError> {
    sqlx::query(
...
pub async fn get(pool: &SqlitePool, job_id: uuid::Uuid) -> Result<Option<ReindexJob>, StoreError> {
    let row: Option<JobRow> = sqlx::query_as(
        "SELECT job_id, status, workspace_id, embedding_model,
                processed, failed, error, started_at, finished_at
         FROM maidan_reindex_jobs WHERE job_id = ?",
    )
    .bind(job_id)
    .fetch_optional(pool)
    .await?;

...
impl WorkspaceStore for SqliteStore {
    ...
    async fn list_legal_holds(&self) -> Result<Vec<LegalHold>, StoreError> {
        legal_hold::list(&self.pool).await
    }
...
impl MemberStore for SqliteStore {
    ...
    async fn get_scim_user(&self, member_id: MemberId) -> Result<Option<ScimUser>, StoreError> {
        scim_users::get(&self.pool, member_id).await
    }
    async fn list_scim_users(
        &self,
        workspace_id: WorkspaceId,
    ) -> Result<Vec<ScimUser>, StoreError> {
        scim_users::list(&self.pool, workspace_id).await
    }
...
impl SkillStore for SqliteStore {
    ...
    async fn has_worked_thread(
        ...
        thread_workers::has_worked(&self.pool, thread_id, member_id).await
    }

    async fn list_thread_workers(&self, thread_id: ThreadId) -> Result<Vec<MemberId>, StoreError> {
        thread_workers::list_workers(&self.pool, thread_id).await
    }
...
impl ThreadResultStore for SqliteStore {
    ...
    async fn list_channel_closed_results(
        ...
        thread_results::list_closed_in_channel(&self.pool, channel_id, exclude_thread_id, limit)
            .await
    }
}
...
impl BudgetStore for SqliteStore {
    ...
    async fn get_thread_budget(
        ...
        budget::get_budget(&self.pool, thread_id).await
    }
    ...
    async fn list_channel_dlq(
        ...
        dlq::list_for_channel(&self.pool, channel_id, limit).await
    }
}

#[async_trait]
impl UsageLedgerStore for SqliteStore {
    ...
    async fn get_usage_ledger_entry(
        ...
        usage_ledger::get(&self.pool, usage_report_id).await
    }
    ...
    async fn list_thread_usage_ledger(
        ...
        usage_ledger::list_for_thread(&self.pool, thread_id, limit).await
    }
}
...
impl GlossaryStore for SqliteStore {
    ...
    async fn get_glossary_term(
        ...
        glossary::get(&self.pool, workspace_id, term).await
    }
    async fn list_glossary_terms(
        &self,
        workspace_id: WorkspaceId,
    ) -> Result<Vec<GlossaryTerm>, StoreError> {
        glossary::list(&self.pool, workspace_id).await
    }
...
impl FollowStore for SqliteStore {
    ...
    async fn is_thread_muted(
        ...
        follows::is_thread_muted(&self.pool, member_id, thread_id).await
    }
    async fn thread_muters(&self, thread_id: ThreadId) -> Result<Vec<MemberId>, StoreError> {
        follows::thread_muters(&self.pool, thread_id).await
    }
    ...
    async fn is_channel_muted(
        ...
        follows::is_channel_muted(&self.pool, member_id, channel_id).await
    }
    async fn channel_muters(&self, channel_id: ChannelId) -> Result<Vec<MemberId>, StoreError> {
        follows::channel_muters(&self.pool, channel_id).await
    }
}
...
#[async_trait]
impl EgressStore for SqliteStore {
...
impl ThreadStore for SqliteStore {
    ...
    async fn page_threads_for_channel(
        ...
        threads::page_for_channel(&self.pool, channel_id, after, limit).await
    }
...
impl ReviewStore for SqliteStore {
    ...
    async fn get_review_requirement(
        ...
        reviews::get_requirement(&self.pool, thread_id).await
    }
    ...
    async fn list_reviewers(&self, thread_id: ThreadId) -> Result<Vec<MemberId>, StoreError> {
        reviews::list_reviewers(&self.pool, thread_id).await
    }
...
#[async_trait]
impl MemoryBlockStore for SqliteStore {
    ...
    async fn get_memory_block(&self, id: MemoryBlockId) -> Result<Option<MemoryBlock>, StoreError> {
        memory_blocks::get(&self.pool, id).await
    }
    async fn get_memory_block_by_label(
        ...
        memory_blocks::get_by_label(&self.pool, workspace_id, label).await
    }
    async fn list_memory_blocks(
        &self,
        workspace_id: WorkspaceId,
    ) -> Result<Vec<MemoryBlock>, StoreError> {
        memory_blocks::list(&self.pool, workspace_id).await
    }
    ...
    async fn list_thread_memory_blocks(
        ...
        memory_blocks::list_for_thread(&self.pool, thread_id).await
    }
}
...
#[async_trait]
impl IntegrityStore for SqliteStore {
    ...
    async fn list_message_backlinks(
        ...
        explorer::list_message_backlinks(&self.pool, message_id).await
    }
...
#[async_trait]
impl ShareTicketStore for SqliteStore {
    ...
    async fn get_share_ticket(&self, id: ShareTicketId) -> Result<ShareTicket, StoreError> {
        share_tickets::get(&self.pool, id).await
    }
    ...
    async fn list_share_tickets(
        ...
        share_tickets::list(&self.pool, workspace_id).await
    }
    ...
    async fn resolve_share_ticket(
        ...
        share_tickets::resolve(&self.pool, token_hash, now).await
    }
    ...
    async fn list_share_ticket_artifacts(
        ...
        share_tickets::list_artifacts(&self.pool, id).await
    }
...
#[async_trait]
impl GovernanceAuditStore for SqliteStore {

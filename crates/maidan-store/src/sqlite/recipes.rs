...
fn row_to_run(row: &sqlx::sqlite::SqliteRow) -> Result<RecipeRun, StoreError> {
    let params: String = row.get("params");
    let snapshot: String = row.get("spec_snapshot");
    Ok(RecipeRun {
        ...
        spec_snapshot: serde_json::from_str::<RecipeSpec>(&snapshot)?,
        created_by: MemberId(row.get::<Uuid, _>("created_by")),
        created_at: row.get::<DateTime<Utc>, _>("created_at"),
    })
}
...
pub async fn instantiate(
    ...
    let snapshot = serde_json::to_string(&recipe.spec)?;
...
pub async fn get_run(pool: &SqlitePool, id: RecipeRunId) -> Result<RecipeRun, StoreError> {
    let row = sqlx::query(&format!(
        "SELECT {RUN_COLS} FROM maidan_recipe_runs WHERE id = ?"
    ))
    .bind(id.0)
    .fetch_optional(pool)
    .await?
...
pub async fn latest_run(
    ...
    let row = sqlx::query(&format!(
        "SELECT {RUN_COLS} FROM maidan_recipe_runs WHERE recipe_id = ? ORDER BY created_at DESC, id LIMIT 1"
    ))
    .bind(recipe_id.0)
    .fetch_optional(pool)
    .await?;

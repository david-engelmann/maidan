...
/// Gate a `closed` transition (SQLite twin). Refuses close until the review
/// requirement is met — `k` distinct **qualifying** approvals (decision =
/// approve, reviewer is neither owner nor assignee nor a past holder, and, when
/// a named reviewer set exists, is in it) — and no unresolved `refutes`
...
async fn review_gate_in_tx(
    ...
    let row = sqlx::query(
        "SELECT
           COALESCE((SELECT required_count FROM maidan_thread_review_reqs WHERE thread_id = ?), 0)
             AS required_count,
           (SELECT COUNT(*) FROM maidan_thread_reviews r
              JOIN maidan_threads t ON t.id = r.thread_id
              WHERE r.thread_id = ? AND r.decision = 'approve' AND r.dismissed_at IS NULL
    ...
    let refuted = sqlx::query(
        "SELECT 1 FROM maidan_references
         WHERE relation = 'refutes' AND dst_kind = 'thread' AND dst_id = ? LIMIT 1",
    )
    .bind(thread_id.0)

...
pub async fn get(pool: &PgPool, surface: &str) -> Result<i64, StoreError> {
    let row = sqlx::query("SELECT last_event_id FROM maidan_tap_cursor WHERE surface = $1")
        .bind(surface)
        .fetch_optional(pool)
        .await?;
...
pub async fn set(pool: &PgPool, surface: &str, last_event_id: i64) -> Result<(), StoreError> {
    sqlx::query(
        "INSERT INTO maidan_tap_cursor (surface, last_event_id, updated_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (surface) DO UPDATE
           SET last_event_id = GREATEST(maidan_tap_cursor.last_event_id, excluded.last_event_id),
...
pub async fn clear(pool: &PgPool, surface: &str) -> Result<(), StoreError> {
    sqlx::query("DELETE FROM maidan_tap_cursor WHERE surface = $1")
        .bind(surface)
        .execute(pool)
        .await?;
    Ok(())

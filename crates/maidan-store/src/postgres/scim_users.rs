...
fn row_to_scim(row: &sqlx::postgres::PgRow) -> ScimUser {
    ScimUser {
...
pub async fn get(pool: &PgPool, member_id: MemberId) -> Result<Option<ScimUser>, StoreError> {
    let row = sqlx::query(&format!(
        "SELECT {COLS} FROM maidan_scim_users WHERE member_id = $1"
    ))
    .bind(member_id.0)
    .fetch_optional(pool)
    .await?;
...
pub async fn list(pool: &PgPool, workspace_id: WorkspaceId) -> Result<Vec<ScimUser>, StoreError> {
    let rows = sqlx::query(&format!(
        "SELECT {COLS} FROM maidan_scim_users WHERE workspace_id = $1
...
pub async fn update(
    ...
    let row = sqlx::query(&format!(
        "UPDATE maidan_scim_users
         SET external_id = $2, active = $3, updated_at = NOW()
         WHERE member_id = $1 RETURNING {COLS}"
    ))
    .bind(member_id.0)
    .bind(external_id)
    .bind(active)
...
pub async fn delete(pool: &PgPool, member_id: MemberId) -> Result<bool, StoreError> {
    let done = sqlx::query("DELETE FROM maidan_scim_users WHERE member_id = $1")
        .bind(member_id.0)
        .execute(pool)
        .await?;

...
pub async fn delete(pool: &PgPool, id: PeerId) -> Result<(), StoreError> {
    let result = sqlx::query("DELETE FROM maidan_peers WHERE id = $1")
        .bind(id.0)
        .execute(pool)
        .await?;
...
pub async fn ingest_exists(
    ...
    let row: Option<(i64,)> = sqlx::query_as(
        "SELECT 1 FROM maidan_federated_ingest
         WHERE peer_id = $1 AND remote_event_id = $2
         LIMIT 1",
    )
    .bind(peer_id.0)
...
pub async fn try_record_ingest(
    ...
    let result = sqlx::query(
        "INSERT INTO maidan_federated_ingest
            (peer_id, remote_event_id, local_event_id, origin_prev_hash, origin_content_hash)
         VALUES ($1, $2, $3, $4, $5)
...
pub async fn is_federated_local_event(
    ...
    let row: Option<(i64,)> =
        sqlx::query_as("SELECT 1 FROM maidan_federated_ingest WHERE local_event_id = $1 LIMIT 1")
            .bind(local_event_id)
            .fetch_optional(pool)
            .await?;
    Ok(row.is_some())
...
fn row_to_peer(row: &sqlx::postgres::PgRow) -> Result<Peer, StoreError> {
    Ok(Peer {

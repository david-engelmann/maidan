...
use crate::postgres::{channels, threads};
...
pub async fn get(pool: &PgPool, id: DmConversationId) -> Result<DmConversation, StoreError> {
    let row = sqlx::query(
        "SELECT id, workspace_id, member_low_id, member_high_id, thread_id, created_at, updated_at
         FROM maidan_dm_conversations WHERE id = $1",
    )
    .bind(id.0)
    .fetch_optional(pool)
    .await?
...
pub async fn get_by_members(
    ...
    let row = sqlx::query(
        "SELECT id, workspace_id, member_low_id, member_high_id, thread_id, created_at, updated_at
         ...
         WHERE workspace_id = $1 AND member_low_id = $2 AND member_high_id = $3",
    )
    .bind(workspace_id.0)
    .bind(low.0)
    .bind(high.0)
...
pub async fn list_for_member(
    ...
    let rows = sqlx::query(
        "SELECT id, workspace_id, member_low_id, member_high_id, thread_id, created_at, updated_at
         FROM maidan_dm_conversations
         WHERE workspace_id = $1 AND (member_low_id = $2 OR member_high_id = $2)
...
pub async fn open(
    ...
    let row = sqlx::query(
        "INSERT INTO maidan_dm_conversations
            (id, workspace_id, member_low_id, member_high_id, thread_id)
         VALUES ($1, $2, $3, $4, $5)
...
pub async fn get_for_thread(
    ...
    let row = sqlx::query(
        "SELECT id, workspace_id, member_low_id, member_high_id, thread_id, created_at, updated_at
         FROM maidan_dm_conversations WHERE thread_id = $1",
    )
    .bind(thread_id.0)
    .fetch_optional(pool)
    .await?;
...
fn row_to_dm(row: &sqlx::postgres::PgRow) -> DmConversation {
    DmConversation {

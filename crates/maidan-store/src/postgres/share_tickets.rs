...
pub async fn create(pool: &PgPool, new: NewShareTicket) -> Result<ShareTicket, StoreError> {
...
pub async fn get(pool: &PgPool, id: ShareTicketId) -> Result<ShareTicket, StoreError> {
    let row = sqlx::query(&format!(
...
pub async fn revoke(
    ...
    let result = sqlx::query(
        "UPDATE maidan_share_tickets SET revoked_at = NOW()
...
pub async fn list_artifacts(pool: &PgPool, id: ShareTicketId) -> Result<Vec<String>, StoreError> {
...
fn row_to_ticket(row: &sqlx::postgres::PgRow) -> ShareTicket {
    ShareTicket {

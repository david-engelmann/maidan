...
pub async fn replace(
    ...
    sqlx::query("DELETE FROM maidan_token_quotas WHERE token_id = $1")
        .bind(token_id.0)
        .execute(&mut *tx)
        .await?;
    for q in quotas {
...
pub async fn list(pool: &PgPool, token_id: ApiTokenId) -> Result<Vec<TokenQuota>, StoreError> {
...
fn row_to_quota((capability, max, window): (String, i32, i32)) -> Result<TokenQuota, StoreError> {
    if max <= 0 || window <= 0 {

//! Slash-command dispatch, dependency-inverted.
//!
...
//! webhook client, secret decryption, and re-entry into the MCP server). The
//! MCP `post_message` handler lives here in `maidan-mcp`, which `maidan-server`
//! depends on (not the reverse), so it can't call that logic directly.
//!
//! [`SlashDispatcher`] inverts the dependency: `maidan-server` implements it
//! and attaches an instance to the [`McpServer`](crate::server::McpServer) at
//! startup (`set_slash_dispatcher`), and the MCP post path invokes it when a
//! slash command is registered — giving MCP posts the same slash behaviour as
//! REST posts.
